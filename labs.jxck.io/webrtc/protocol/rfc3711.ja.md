...
この文書では、 RTP のプロファイルとしての SRTP を定義する。
これは、機密性、メッセージ認証、リプレイ保護、を RTP/RTCP に提供する。
...
SRTP は、暗号とメッセージ認証を RTP/RTCP Stream に提供するためのフレームワークを提供する。
SRTP は、デフォルトの暗号変換を定義し、将来の拡張も可能。
鍵管理があるため、 uni/multicast でセキュア。
...
RTP は、高いスループットと低パケット拡張を実現する。
SRTP は、異機種環境(有線/無線ネットワークの混在)に適した保護を提供。
このような特徴を得るため、デフォルト変換は考えられている。
暗号がストリーム暗号に基づく。
鍵付きハッシュベースの、メッセージ認証関数
そして、 RTP の SeqNum から算出する "暗黙の index" を、SRTP/SRTCP でシーケンス/同期に使う。
...
我々は、用語 "メッセージ認証" と "認証タグ" という言葉を使います。
...
- 全体の RTP と RTCP パケットの完全性、およびリプレイパケットに対する保護機能
...
これらセキュリティ機能は、オプションかつ互いに独立している。
ただし、 SRTCP 完全保護は必須。
(そうでない場合は RTP ストリームの処理を妨害する可能性が RTCP メッセージの悪意のあるまたは誤った変更)
...
- "Master Key" は、SRTP/SRTCP ストリームに、 Key Material を提供することができます。 これは KDF (4.3) から取得される "Session Keys"
...
RTP のプロファイルとして SRTP を定義します。
このプロファイルは、 RTP オーディオ/ビデオプロフィール [RFC3551] の拡張です。
明示的に注記がある場合を除き、そのプロファイルのすべての側面は、 SRTP セキュリティ機能を追加して適用されます。
概念的に SRTP を RTP アプリケーションとトランスポート層との間に存在する "bump in the stack" 実装であると考えています。
SRTP は RTP パケットを横取りし、等価な SRTP パケットを送信し、受信した SRTP パケットを、等価な RTP パケットにして受信側に渡します。
...
"Encrypted Portion" は、対応する RTP パケットのPayload(+padding)を暗号化した SRTP パケット。
...
事前定義された暗号化 transform のいずれも、パディングを使用しません。そのため RTP および SRTP ペイロードサイズが正確に一致します。
...
MKI は、特定のパケットを認証/暗号化した Session Key が導出された Master Key を識別する。

Note that the MKI SHALL NOT identify the SRTP cryptographic context, which is identified according to Section 3.2.3.

MKI は、 Section 3.2.3 に従って同定される SRTP 暗号コンテキストを識別しないことに注意してください。
...
MKI が、保護整合性ではないことにより、追加の保護を提供しないことに注意してください。
...
master key は、暗号的に安全な方法で session key を導出した、(key management protocol により提供された)ランダムなビット列を意味します。
...
##### ROC
 ...
 32 ビット符号なしロールオーバカウンタ(ROC) (16 bit の RTP SEQ が 65,535 から 0 にリセットされた回数を記録する。 3.3.1 に記載のように RTP パケットヘッダから抽出される SRTP SEQ とは異なり、 ROC は SRTP によって維持されます。)
...
レシーバのみ、 `s_l` という 16bit のシーケンスを保持する、これは、メッセージ認証された受信した中で一番大きい RTP SeqNum (3.3.1)
...
##### message authentication algorithm id
...
(認証およびリプレイ保護が提供されている)レシーバによってメンテされる再生リスト。
最近受信した認証 SRTP パケットのインデックスを含む。
...
MKI インジケータが 1 に設定されている場合、 MKI のオクテット長、 Sender で現在アクティブな MKI
(MKI インジケータと length の値は、コンテキストが維持された間は、固定でなければならない)
...
各マスターキーで、そのマスターキーを用いて(送信)処理された SRTP パケットの数のカウント(セキュリティのために不可欠 section 3.3.1 and 9)
...
n_e / n_a(非負整数) は、暗号化、およびメッセージ認証のためのセッションキーの長さを決定します。
...
a master salt, to be used in the key derivation of session keys. This value, when used, MUST be random, but MAY be public.
Use of master salt is strongly RECOMMENDED, see Section 9.2. A "NULL" salt is treated as 00...0.

Master Salt は鍵導出で Session Key を出すのに使う。
乱数で公開可能な値。
Salt の利用は強く推奨され、 NULL の場合は全部 `0`


##### Key Derivation Rate

an integer in the set {1,2,4,...,2^24}, the "keyderivationrate", where an unspecified value is treated as zero.
The constraint to be a power of 2 simplifies the session-key derivation implementation, see Section 4.3.

`1...2^24` 内の整数で、それ以外の値は `0` として扱われる。
ただし、 2 の累乗にしておくと、 Session Key の導出がかんたんになる。(4.3)
...
expressed in terms of the two 48-bit index values inside whose range (including the range end-points) the master key is valid.

48-bit のインデックス値 2 つで、レンジの中の値が正しい。

For the use of `<From, To>`, see Section 8.1.1.

8.1.1 参照

`<From, To>` is an alternative to the MKI and assumes that a master key is in one-to-one correspondence with the SRTP session key on which the `<From, To>` range is defined.
...
SRTCP は、以下を除いて SRTP の暗号コンテキストを共有します
...
- SRTCP は index を含むので、 ROC と s_l はいらない
- (再生保護が提供される場合)再生リストは別で維持されます
- SRTCP は、そのキーで処理された SRTCP パケットの数のカウントを維持するための手段として、その Master Key(SRTP と同じであっても)と別個のカウンタを維持します。
...
事前定義の変換がある場合 Master Key は SRTP/SRTCP で共有されるが、 Session Key は共有されない。
...
加えて、 RTP セッションの SRTP Stream が複数ある場合がある

identified by their synchronization source (SSRCs, which is part of the RTP header),

SSRC で識別される


share most of the crypto context parameters (including possibly master and session keys).

ここでは、多くの暗号化パラメータを共有する

In such cases, just as in the normal SRTP/SRTCP parameter sharing above,

その場合、 SRTP/SRTCP のパラメータも共有される。

separate replay lists and packet counters for each stream (SSRC) MUST still be maintained. Also, separate SRTP indices MUST then be maintained.

でも、リプレイリストとパケットカウンターは SSRC ごとに管理。
...
All encryption, authentication/integrity, and key derivation parameters are defined in the transforms section (Section 4).

全ての暗号化、認証/完全性、および鍵導出パラメータは変換に定義されています(section 4)。

Typical examples of such parameters are block size of ciphers, session keys, data for the Initialization Vector (IV) formation, etc.

パラメータは例えば、暗号の block size/session keys/IV 用のデータ etc

Future SRTP transform specifications MUST include a section to list the additional cryptographic context's parameters for that transform, if any.

SRTP 変換を追加する場合、このセクションを定義する。


#### 3.2.3. 暗号コンテキストと SRTP パケットのマッピング (3.2.3. Mapping SRTP Packets to Cryptographic Contexts)

Recall that an RTP session for each participant is defined [RFC3550] by a pair of destination transport addresses (one network address plus a port pair for RTP and RTCP),

各参加者の RTP セッションは宛先アドレスのペアで定義され

and that a multimedia session is defined as a collection of RTP sessions.

マルチメディアセッションは RTP session の集合であるため

For example, a particular multimedia session could include an audio RTP session,

たとえば、特定のマルチメディアセッションは RTP の audio/video/text session を含むことができる

A cryptographic context SHALL be uniquely identified by the triplet context identifier:

暗号コンテキストを 3 つの識別子によって識別されなければなりません。

```
context id = <SSRC, dest address, dest port>
```
...
宛先アドレスとポートは SRTP パケットの一つ。
なお、この情報を提供するとき、 3.2 で説明したように、鍵管理情報とコンテキストを返すことが想定されます。

As noted above, SRTP and SRTCP by default share the bulk of the parameters in the cryptographic context.
Thus, retrieving the crypto context parameters for an SRTCP stream in practice may imply a binding to the correspondent SRTP crypto context.

前述の通り、 SRTP/SRTCP は暗号化パラメータを共有する。

It is up to the implementation to assure such binding,
since the RTCP port may not be directly deducible from the RTP port only.

バインディングの方法は実装による
RTCP はポートだけでわからないので


Alternatively, the key management may choose to provide separate SRTP- and SRTCP- contexts,
duplicating the common parameters (such as master key(s)).

代わりに、鍵管理は SRTP/SRTCP コンテキストを分離する
共有パラメータを複製する

The latter approach then also enables SRTP and SRTCP to use, e.g., distinct transforms, if so desired.
Similar considerations arise when multiple SRTP streams, forming part of one single RTP session, share keys and other parameters.

他のアプローチとして、 SRTP/SRTCP を別の変換で行う
別 SRTP-と SRTCP-コンテキストを提供するように選択することができます。後者のアプローチは、その後も、所望であれば、例えば、使用する SRTP と SRTCP を可能にする別個の変換。複数 SRTP ストリームは、単一の RTP セッション、共有キーおよびその他のパラメータの一部を形成する場合、同様の考慮事項が生じます。
...
1. Section 3.2.3 で説明したように使用する暗号コンテキストを決定します。
...
2. Section 3.3.1 のように、ロールオーバカウンタ、暗号コンテキストにおける最大シーケンス番号、 RTP パケットのシーケンス番号を使用して、 SRTP パケットのインデックスを決定します。
...
3. Master Key と Master Salt を決定します。これは、前のステップで決定されたインデックスまたは、 Section 8.1 のように暗号コンテキストにおける現在の MKI を使用して行われます。

4. Determine the session keys and session salt (if they are used by the transform) as described in Section 4.3, using master key, master salt, keyderivationrate, and session key-lengths in the cryptographic context with the index, determined in Steps 2 and 3.

4. (transform で利用する場合) Session Key, Session Salt を決定します。導出は Section 4.3 のように、 master key, master salt, key derivation rate, session key-length, index を使います。
...
5. RTP ペイロードの暗号化をする。暗号化アルゴリズム, Index, Session key/salt が使われる。

6. If the MKI indicator is set to one, append the MKI to the packet.

6. MKI インジケータが 1 に設定されている場合は、パケットに MKI を追加します。
...
7. メッセージ認証のために、 4.2 節で説明したように、パケットの認証部分についての認証タグを計算します。このステップは、現在のロールオーバカウンタ、暗号コンテキストにの認証アルゴリズム、 step 4 で見つかった session authentication key で計算し、パケットに追加します。

8. If necessary, update the ROC as in Section 3.3.1, using the packet index determined in Step 2.

8. 必要であれば、ステップ 2 で決定されたパケットインデックスを使用して、セクション 3.3.1 のように ROC を更新します。
...
1. Section 3.2.3 で説明したように、使用する暗号コンテキストを決定します。
...
2. SRTP パケットのインデックスを取得する(シーケンス番号, ROC, シーケンス番号の最高値, SRTP パケットのシーケンス番号, を使用)。
...
3. Master Key/Salt を決定します。コンテキストにおける MKI インジケータが 1 に設定されている場合は、SRTP パケットに MKI を使用します、そうでない場合は、 8.1 のように前のステップからのインデックスを使用します。

4. Determine the session keys, and session salt (if used by the transform) as described in Section 4.3, using master key, master salt, keyderivationrate and session key-lengths in the cryptographic context with the index, determined in Steps 2 and 3.

4. Session Key/Salt (4.3)、暗号コンテキストの master key / master salt / keyderivationrate / session key length を index と一緒に使う。(step 2, 3).

5. For message authentication and replay protection, first check if the packet has been replayed (Section 3.3.2), using the Replay List and the index as determined in Step 2. If the packet is judged to be replayed, then the packet MUST be discarded, and the event SHOULD be logged. Next, perform verification of the authentication tag, using the rollover counter from Step 2, the authentication algorithm indicated in the cryptographic context, and the session authentication key from Step 4. If the result is "AUTHENTICATION FAILURE" (see Section 4.2), the packet MUST be discarded from further processing and the event SHOULD be logged.

5. メッセージ認証とリプレイ保護のため、最初に Step 2 で決定したリプレイリストとインデックスを用いて、パケットがリプレイされているかをチェックする(3.3.2)。
もしパケットがリプレイされたものだとわかれば、パケットは無視され、ログを残す。 次に、ステップ 2 からロールオーバーカウンターを使用して、認証タグの検証を実行し、暗号コンテキストに示される認証アルゴリズム、及び結果が「認証失敗」である場合、ステップ 4 からセッション認証キー(セクション 4.2 を参照)、パケットは、さらなる処理から捨てなければなりませんし、イベントがログに記録されます。
...
6. パケットの復号化 (algorithm, session key/salt, index)
...
7. 3.3.2 で説明したようにリプレイリストを更新し、再生保護が提供されている場合は、ステップ 2 で推定したパケットインデックスを使用して、セクション 3.3.1 のように ROC と最高のシーケンス番号, s_l を更新します。

8. When present, remove the MKI and authentication tag fields from the packet.

8. MKI と auth tag を削除
...
SRTP ではパケットの並びはパケットにある値から index を計算して使います。
pre-defined transform では、 index はリプライ保護(3.3.2)、暗号化(4.1)、メッセージ認証(4.2)、鍵導出(4.3) に使われます。
...
セッションの開始時に、 Sender 側は ROC を 0 にし、 RTP の Sequence Number が 2^16 を超えたら、 ROC を 1 増やす(かつ 2^32 を超えたら 0)。
結果、 index はこうなる。
...
Receiver では、 RTP の SeqNum をパケットの順番の確認として使う。
ROC を適切に使うには、明確な定義が必要。例えば、 2^16 や 0 に近いパケットの順序が変わってしまった場合など。
...
index の推定は、 Receiver がローカルに持つ ROC と sl の値を用いる。
セッションのセットアップ時、 ROC = 0 とする。
すでに走っているセッションに Receiver が参加する場合は、シグナリングの方で現在の ROC を教える。
Receiver は sl を、最初に受信した SRTP パケットの RTP SEQ にする。(シグナリングで初期値が来た場合はそれ)

On consecutive SRTP packets, the receiver SHOULD estimate the index as

連続する SRTP パケットに対して、 Receiver は以下のようにインデックスを推定する
...
v は `{ ROC-1, ROC, ROC+1 }(mod 2^32)` のなかで、 i が一番 `2^16 * ROC + s_l` に近いもの(Appendix A)。
...
パケットが処理され、認証されたら、 Receiver は v を使って sl と ROC を更新する。
`v = (ROC-1) mod 2^32` ならアップデートなし。
`v = ROC` なら、 `sl > SEQ` の場合のみ `sl = SEQ` で ROC はそのまま。
`v = (ROC+1) mod 2^32` なら、 `sl = SEQ` で `ROC = v`
...
re-keying (master key の更新)が発生しても、 ROC はリセットせずそのまま。
...
As the rollover counter is 32 bits long and the sequence number is 16 bits long,
ROC は 32 bit, SEQ は 16 bit である。

the maximum number of packets belonging to a given SRTP stream that can be secured with the same key is 2^48 using the pre-defined transforms.
同じ Key で暗号化できるパケット数は、pre-defined transform では 2^48。

After that number of SRTP packets have been sent with a given (master or session) key, the sender MUST NOT send any more packets with that key.
パケット数がそれを超えたら、その鍵はもう使えない。

(There exists a similar limit for SRTCP, which in practice may be more restrictive, see Section 9.2.)
(SRTCP も同様のリミットがある 9.2)

This limitation enforces a security benefit by providing an upper bound on the amount of traffic that can pass before cryptographic keys are changed.
一定以上で鍵を変える必要があるこの制限は、セキュリティ上のメリットがある。

Re-keying (see Section 8.1) MUST be triggered, before this amount of traffic, and MAY be triggered earlier, e.g.,for increased security and access control to media.
制限に近づいたら Re-Keying (8.1) を行う必要がある。


Recurring key derivation by means of a non-zero keyderivationrate (see Section 4.3), also gives stronger security but does not change the above absolute maximum value.

Key Derivation が繰り返し起こる(4.3)と、より強力なセキュリティを提供できる。


On the receiver side, there is a caveat to updating sl and ROC:
Receiver では、 sl と ROC の更新には注意点がある。

if message authentication is not present, neither the initialization of sl, nor the ROC update can be made completely robust.
もしメッセージ認証が無い場合、 sl の初期化も、 ROC の更新も完全に堅牢とは言えない。

The receiver's "implicit index" approach works for the pre-defined transforms
as long as the reorder and loss of the packets are not too great and bit-errors do not occur in unfortunate ways.

Receiver が暗黙的な index を使うアプローチは、 pre-defined transform で動くのは
パケットの reorder と loss が影響しても bit-errors 、想定しないことがおきない。

In particular, 2^15 packets would need to be lost, or a packet would need to be 2^15 packets out of sequence before synchronization is lost.
具体的には、同期が失われるには 2^15 の loss か reorder が起こる。

Such drastic loss or reorder is likely to disrupt the RTP application itself.
ここまで大幅な loss や reorder は、 RTP アプリ自体を落とす。


The algorithm for the index estimate and ROC update is a matter of implementation,
and should take into consideration the environment (e.g., packet loss rate)
and the cases when synchronization is likely to be lost,
e.g., when the initial sequence number (randomly chosen by RTP) is not known in advance (not sent in the key management protocol) but may be near to wrap modulo 2^16.
...
SRTP で使用可能な暗号・認証アルゴリズムは多くあるので、デフォルトを設定する。
拡張の仕方は後述。
...
- BLOCK_CIPHER-MODE は、ブロック暗号が使用されることと、その動作モードを示します。
- `n_b` ブロック暗号のブロックサイズ
- `k_e` session encryption key
- `n_e` k_e の長さ
- `k_s` session salting key
- `n_s` k_s の長さ
- SRTP_PREFIX_LENGTH は、キーストリームのプレフィックスの長さ、非負整数、利用されるメッセージ認証で定義



The distinct session keys and salts for SRTP/SRTCP are by default derived as specified in Section 4.3.
SRTP / SRTCP のための個別の Session Key/Salt は、 4.3 節でデフォルトが指定されます。

The encryption transforms defined in SRTP map the SRTP packet index and secret key into a pseudo-random keystream segment.
SRTP に定義される暗号変換は、 SRTP パケットインデックスと、 Secret Key の Pseudo-Random Keystream Sgment をマップします。

Each keystream segment encrypts a single RTP packet.
各 Keystream Segment が単一の RTP パケットを暗号化します。

The process of encrypting a packet consists of generating the keystream segment corresponding to the packet,
パケットの暗号化プロセスは、パケットに対応する Keystream Segment の生成を含む。

and then bitwise exclusive-oring that keystream segment onto the payload of the RTP packet to produce the Encrypted Portion of the SRTP packet.
そして、 Keystream Segment と RTP パケットのペイロードを XOR して、 SRTP 暗号化パケットを作る。

In case the payload size is not an integer multiple of n_b bits,
ペイロードサイズが `n_b` bits の倍数でない場合、

the excess (least significant) bits of the keystream are simply discarded.
キーストリームの余剰(最下位)ビットは無視される。

Decryption is done the same way, but swapping the roles of the plaintext and ciphertext.
復号化も、暗号と平文を入れ替える以外は同じ。
...
The definition of how the keystream is generated, given the index, depends on the cipher and its mode of operation.

KeyStream の生成方法と、 index の与え方は、暗号化とモードによって変わる。

Below, two such keystream generators are defined. The NULL cipher is also defined, to be used when encryption of RTP is not required.

以下は、 2 つの KeyStream 生成が定義されている。暗号化しない NULL Cipher もある。

The SRTP definition of the keystream is illustrated in Figure 3.

SRTP の keystream 生成は Figure3 にある。

The initial octets of each keystream segment MAY be reserved for use in a message authentication code,

KeyStream Segment の初期オクテットは、メッセージ認証のコードのために予約されている。

in which case the keystream used for encryption starts immediately after the last reserved octet.

暗号化に KeyStream を使う場合は、予約されたオクテットの後をつかう。

The initial reserved octets are called the "keystream prefix" (not to be confused with the "encryption prefix" of [RFC3550, Section 6.  1]),

最初の予約オクテットは "keystream prefix" という。

and the remaining octets are called the "keystream suffix".

それ以降を "keystream suffix" という。

The keystream prefix MUST NOT be used for encryption.

Keystream prefix は暗号には使わない。
...
The number of octets in the keystream prefix is denoted as SRTPPREFIXLENGTH.

keystream prefix のサイズは SRTPPREFIXLENGTH で示す

The keystream prefix is indicated by a positive,non-zero value of SRTPPREFIXLENGTH.

keystream prefix は非ゼロの正の整数

This means that, even if confidentiality is not to be provided,

つまり、機密性が必要なくても、

the keystream generator output may still need to be computed for packet authentication,

KG は、認証のためのパケットを生成する必要がある。

in which case the default keystream generator (mode) SHALL be used.
...
and we define two modes of running AES,

AES 用のモードとして 2 つある

(1) Segmented Integer Counter Mode AES
(2) AES in f8-mode.
...
`E(k, x)` は key k と block x を AES に適用したことを意味する。
...
キーは `k=Enc Key` を利用し、ブロックインデックッスは昇順に並べられる。
...
128 ビットの整数値 IV は、 SSRC, SRTP packet index i 、および SRTP Salt Key K_s によって以下のように定義されます。
...
SSRC を含むのは、同じ key を同一セッションの別 SRTP に適用できるようにするため。(9.1)
...
SRTCP の場合、合成したパケットの最初のヘッダの SSRC を使う。
i は SRTCP index で Enc/Salt key も SRCTP のものを使う。

Note that the initial value, IV, is fixed for each packet and is formed by "reserving" 16 zeros in the least significant bits for the purpose of the counter.
初期値 IV は、パケット毎に固定され、カウンタために、最下位ビットに 16bit の 0 が reserved とされています。

The number of blocks of keystream generated for any fixed value of IV MUST NOT exceed 2^16 to avoid keystream re-use, see below.
IV のために生成される keystream のブロクサイズは、再利用を防ぐため 2^16 を超えてはならない。

The AES has a block size of 128 bits,
AES は 128bit ブロクサイズ

so 2^16 output blocks are sufficient to generate the 2^23 bits of keystream needed to encrypt the largest possible RTP packet
なので、 2^16 のブロック出力は RTP の最大である 2^23 のキーストリームを暗号化するのには十分。

(except for IPv6 "jumbograms" [RFC2675], which are not likely to be used for RTP-based multimedia traffic).
This restriction on the maximum bit-size of the packet that can be encrypted ensures the security of the encryption method by limiting the effectiveness of probabilistic attacks [BDJR].
例外もあるが気にしない。

For a particular Counter Mode key,
each IV value used as an input MUST be distinct,
in order to avoid the security exposure of a two- time pad situation (Section 9.1).
To satisfy this constraint,
an implementation MUST ensure that the combination of the SRTP packet index of ROC || SEQ,
and the SSRC used in the construction of the IV are distinct for any particular key.
The failure to ensure this uniqueness could be catastrophic for Secure RTP.
This is in contrast to the situation for RTP itself,
which may be able to tolerate such failures.
It is RECOMMENDED that,
if a dedicated security module is present,
the RTP sequence numbers and SSRC either be generated or checked by that module
(i.e., sequence-number and SSRC processing in an SRTP system needs to be protected as well as the key).
...
採用されている暗号化やメッセージ認証変換にかかわらず、 SRTP key derivation で session key を生成する。
key derivation rate がセッションの開始時に伝達されたら、それ以降両者で通信は必要ない。
...
Key Derivation は一回は必ずやる。
それ以降は "Key Derivation Rate" による。
`r > 0` の場合、 `index mod r = 0` のときはいつでも行われ、 session key の "refreshing" とみなされる。
KDR は master key に対して固定。
...
実装は、暗号変換のために Session Salting Key を導出する、両方の事前定義変換で行われたのと同じ。
...
m(128 以上)と n を正の整数として、 keyed function の集合 `{PRFn(k, x)}` を PRF Family とする。
k は乱数
x は m-bit
PRFn(k,x) は n-bit string で乱数と見分けがつかない
...
`a DIV t` を a 割る t の切り捨てとし、便宜上 `a DIV 0 = 0` とする。
`a DIV t` の結果は a と同じで、ゼロパディング。
...
`<label>` として 8 bit 定数、 Master Salt, KDR, packet index(48bit ROC || SEQ for SRTP)
...
`<labe>` は導出される key の type ごとにユニークで `0x00~0x05`。
このパケットのための n-bit の SRTP key/salt は MasterKey を元に以下のように導出。
...
(PRF は内部で x の format/padding x を定義することも可能 4.3.3)

The session keys and salt SHALL now be derived using:

session key/salt は以下
...
`a DIV t` は実際は 2 進数右シフトで実装できる。
rate が 256 の倍数だとより楽になるが、そこでま規定しない。
...
同じ master key を利用できるパケット数の上限は、 KD とは別の仕様。
...
SRTP の index を 32bit にし、 0 || index (i.e. E-bit を 0-bit に置換)
...
PRF は、
MasterKey が 128/192/256 bit のどれか
block size m = 128
AES CM を使う(4.1.1)
MasterKey と IV (x*2^16)
出力の KeyStream の最初の n bit を使う
...
デフォルトの SRTP transform は必須実装でもある。
pre-defined transform は以下の通り。
...
AES-CM ベースの KD(4.3.1) と、 PRF(4.3.3) で、 128bit Master Key を使うのが、 Session Key 生成のデフォルト。
デフォルトの Master Salt は 112bit で、 KDR は 0
...
SRTP の暗号化コンテキスト(e.g. SRTP master key)を確立する、新しい鍵管理の標準として [MIKEY] [KEYMGT] [SDMS] がある。
プロプライエタリでもオープン・スタンダードでも、鍵管理は telephony ([MIKY], [KINK]) と multicast ([GDOI]) に使われました。
このセクションは、 SRTP セッションの鍵管理のためのガイダンスを提供します。

For initialization,

初期化として、

an interoperable SRTP implementation SHOULD be given the SSRC and MAY be given the initial RTP sequence number for the RTP stream by key management

SRTP の実装は、 SSRC と初期 RTP シーケンスナンバーを、鍵管理による RTP ストリームに提供します。

(thus, key management has a dependency on RTP operational parameters).

(鍵管理は RTP の operational parameter に依存しているため)

Sending the RTP sequence number in the key management may be useful e.g.,

鍵管理への RTP シーケンス番号の送信は有用

when the initial sequence number is close to wrapping (to avoid synchronization problems),

例えば、初期シーケンスナンバーが折り返しそうなとき(同期の問題を回避するため)

and to communicate the current sequence number to a joining endpoint (to properly initialize its replay list).

そして、現在のシーケンス番号でエンドポイントに参加するため(適切にリプレイリストを初期化するため)
...
&lt;From, To&gt;のデフォルト値は、「最初の観測されたパケットから」と「追って通知があるまで」です。しかし、各所与のマスタ/セッションキー(セクション 9.2)の下で送信される SRTP / SRTCP パケットの最大限界が超えてはなりません。
...
場合&lt;From, To&gt;キー検索として使用され、その後、 MKI は、パケットに挿入されていない(と暗号コンテキストにおけるその指標はゼロです)。しかし、 MKI を使用すると同時に、キー寿命<に、より>を使用して排除するものではありません。これは、例えば、 MKI がアクティブにされる時間内にその時点で送信側での信号のに有用です。
...
## 10. 前方誤り訂正メカニズムとの相互作用 (10. Interaction with Forward Error Correction mechanisms)
...
高速再キーイング&lt;From, To&gt;機能では使用しないでください。 SRTCP パケットが再キーイング時間に近い到着した場合は、特に、正しい SRTCP キーを取得する際に問題を与える可能性があります。 MKI は、この場合には、使用されてください。
...
The input block for AES-CM is generated by exclusive-oring the master salt with the concatenation of the encryption key label 0x00 with (index DIV kdr), then padding on the right with two null octets (which implements the multiply-by-2^16 operation, see Section 4.3.3).
