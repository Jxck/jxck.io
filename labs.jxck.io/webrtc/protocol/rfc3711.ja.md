# セキュアリアルタイム転送プロトコル(SRTP) (The Secure Real-time Transport Protocol (SRTP))
...
変換に依存しないパラメータが独立して使用される特定の暗号化または認証変換の暗号コンテキスト内に存在します。 SRTP のための暗号の文脈の変換に依存しないパラメータは、で構成されています。
...
16 ビットの RTP シーケンス番号が 65,535 を通過した後にゼロにリセットされた回数を記録する 32 ビット符号なしロールオーバカウンタ(ROC)。セクション 3.3.1 に記載のように RTP パケットヘッダから抽出 SRTP シーケンス番号(SEQ)とは異なり、 ROC は SRTP によって維持されます。
