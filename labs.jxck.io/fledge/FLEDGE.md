...
2. Seller は、オークションのビジネスロジックに責任を負います。 javascript のコードで、各入札の価格とメタデータを考慮し、「望ましさ(desirability)」のスコアを計算します。 「望ましさ」のスコアが最も高い入札がオークションに勝ちます。
...
Seller はパブリッシャーのページ内で JS の API を起動してオークションを開始します。
