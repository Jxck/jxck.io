...
async function getBold(entry, page) {
  await page.goto(URL)
  const bolds = await page.evaluate(() => {
    return (document.querySelectorAll('em strong')).map((e) => {
...
(async () => {
  ...
  //const URL = "https://blog.jxck.io/"
  const URL = "file:///home/jxck/server/jxck.io/blog.jxck.io/index.html"
  await page.goto(URL)
  ...
  for (let entry of entries) {
    console.log(entry)
    await page.goto(entry, {timeout: 2000,  waitUntil: 'networkidle0'})
    console.log('goto')
    const emstrong = await page.evaluate(() => {
      console.log('evaluate')
      return Array.from(document.querySelectorAll('em, strong')).map((e) => {
        return e.textContent
      })
    })
    bolds.push(emstrong)
    console.log(emstrong)
  }
  const result = Array.from(new Set(bolds.flat().join("").split(""))).sort()
  console.log(result)
