...
`<script>` タグは JavaScript を取得するために GET を発生するが、異なる Origin を記述しても CORS の対象にはならない。これを利用して SOP を迂回する方法が JSONP だ。実際に JSONP を用いて、 CORS を使わずに Intra の情報を SNS で取得してみよう。
...
```html
<!-- SNS 上で Intra の JavaScript を読み込む -->
...
Intra から返った JavaScript は普通に実行される。これを応用すると、次のようなことができる。

SNS が Intra の情報を取得したいと思った場合、 Intra はその情報を次のような JavaScript で提供する。
...
するとブラウザ上では 2 つの JavaScript は同じスコープで評価され、取得したデータが表示できる。

```js
// 2つのJavaScriptは結合したように実行される
...
これによって、 SNS の JavaScript は別 Origin である Intra が提供するデータを取得できた。
...
この場合サーバからは以下のような JavaScript が返る。
...
このように、取得したいデータを関数実行で囲んで(Padding)、 JavaScript として返すことで Origin をまたいだデータのやりとりを CORS なしで行う手法が JSONP(JSON Padding)だ。 JSONP は実装が手軽だったため、 CORS が普及する以前に広まった。
