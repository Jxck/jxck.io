...
:::message alert
この原稿は [Web+DB Press vol.115](https://gihyo.jp/magazine/wdpress/archive/2020/vol115) (2020年2月号) に掲載されたものをベースに、大幅に加筆修正したものです。
:::
...
![図1 Intra と Attack の構成](https://cacoo.com/diagrams/L0Jn5wPiobCrsSDy-98D42.png 'overview')
...
攻撃者が Attack に次のような JavaScript を仕込み、そのページを何らかの方法で社員にアクセスさせることで、 Intra の情報を抜こうとしているとする。
...
社員がこのページにアクセスすると、 JavaScript で生成されたリクエストは、同じ社内ネットワークにある Intra に届いてしまい、取得した結果を Attack に POST で転送すれば、攻撃者はその内容を得ることができてしまう。攻撃者は、社員が Attack を閲覧している間に Intra を JavaScript でクロールして、機密情報をごっそり抜くことも可能になる。
...
一方、 Web の場合は、ページを閲覧するだけで JavaScript が取得され、ネイティブアプリでは行われる配布元の検証といったプロセスを経ずに実行されるため、 URL にアクセスさせるだけで成立してしまう。この攻撃を防ぐには、取得した JavaScript を何の検証もせずに実行しても、情報の流出などが発生しないようにする仕組みが必要だ。
...
先ほど Attack に設置された攻撃用の次の JavaScript は、 Intra に対して GET を送信する`fetch()`が SOP 違反でエラーになるため、情報を収集することはできない。
...
また、❸は http のため経路が暗号化されておらず、中間者によって改ざんされる危険がある。改ざんされた❸に埋め込まれた JavaScript から❶へのリクエストが成功してしまっては、❶が暗号化されている意味がなくなってしまう。
...
ブラウザが受信したレスポンスの内容から **SOP に違反していると判断** し、 JavaScript にはレスポンスを渡さずエラーを上げているからなのだ。
...
**リクエストは送信されているが JavaScript ではエラーになる**、ここが SOP および CORS を理解するうえで最重要な点なので、次説で順を追って解説していく。
