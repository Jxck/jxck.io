# [1password][security] 1Password AC #3: Master Password の作り方
...
今度は「パスワード長の上限」の存在が足かせになる。例えば 14 文字までしか入れられないといった制限は、この前提を大きく壊す。
...
また、「長すぎて覚えられないからどこかにメモする」や「毎回打ってられないのでコピペ用にメモ帳に書いておく」「辞書変換に登録し "パスワード" と打ったら出てくる」といった、別のリスクが誘発される可能性もあるため、ポリシーを 15 文字より長くする理由もそこまでない。
...
そして、このポリシーは組織ごとに設定するため、1 人のユーザが様々な組織に入った場合に、「この組織は 15 文字以上」だが「この組織は 20 文字以上」といった差異が発生することがあり得る。

これまで使ってきた Master Password を共通で使えなくなってしまうと、新しく Master Password を作る必要が出る。複数を運用するのは 1Password の方針とは異なり、またブラウザ拡張やアプリなどでアカウントのスイッチが面倒になるため、新しい Master Password で既存のアカウントを更新する必要が出てしまうのだ。
...
20 文字あれば、小文字アルファベットだけでも `26^20` 通りあり `2x10^28` で *二穣* という単位になる。もはや、記号や数字といった文字種など気にする必要はなく、むしろ絶対に忘れないこと、入力がストレスにならないことを重視して作るべきとなる。
...
特に英語にこだわる必要もない。例えば、吾輩は猫であるをローマ字にすれば 35 文字のパスワードができる。

```
wagahaiha-nekodearu-namaeha-madanai (35)
```

エンジニアであれば、例えば以下のようなコード片を使うと、記号も入って 25 文字になる。手癖で打てるだろう。

```
console.debug(Date.now()) (25)
```

Master Password を作る際は、「20 文字以上の文字列を考える」ではなく、「自分が好きなフレーズをどこかから持ってくる」と考えるのが良いだろう。そうすれば、覚えやすく長い文字列を作ること自体はそこまで難しくない。それが「あからさまに推測しやすいもの」でなければ、長さだけで十分なエントロピーが確保できているため、強度の心配もない。
...
Policy を強制したいのであれば、基本的にはアカウントを配る前に決めておく必要がある点に注意したい。ただし、ユーザが既に持っている Master Password を変更させないとならないほど強いポリシーを後から強制するのは、前述のように避けるべきだ。


## Master Password は誰にも見えない

何度も言うように、1Password の Master Password は「ユーザが記憶のみで管理する」ものなので、1Password を運営する AgileBits であっても、その値を知ることは絶対にできない。
