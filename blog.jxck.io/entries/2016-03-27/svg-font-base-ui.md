...
しかし、 UI アイコンなどについては、 SVG や Font といった **ベクタ形式** を用いる方が、よい場合がある。
...
このため、 [画像最適化戦略 Picture 編](/entries/2016-03-26/picture.html) で実施したようなデバイス解像度による複数画像の出し分けなども不要になる。
...
本サイトのヘッダ部分に表示されているアイコンなどは、基本的にすべて SVG を表示している。

また、自分で作る説明用の図なども、基本的には SVG で作るようにしている。

SVG を生成するツールは、イラストレーターなどがよく使われるようである。

しかし、筆者はイラレを持っておらず、また長いことスライドやブログの画像はすべて cacoo で作成してきた。
...
なお、 PNG と cacoo で書き出した時点での SVG のサイズの比較は以下のとおりである。
...
こうして手書きした SVG は次にソースとともに掲載している。
...
なお、 404 ページや 500 ページを作る際にデザインを考えてたところ、この SVG にリンク貼って返せばそれでよいんじゃないか?と思い、今は単にステータスコードを書いた SVG を返すようにしている。
...
たとえば見出し `<h1>~<h4>` と `pre` は次のように指定している。
