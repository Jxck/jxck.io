...
判別が正しく行えなければ、先述のデフォルト strict mode や、トップレベルの `this` を `undefined` と扱うといった処理ができなくなるからである。
...
パースが正しく終われば、依存解決のために追加の Fetch を以下のように行う。
...
Node.js においては、パース時にそのファイルが ESM/CJS どちらなのかを判別する方法を長く議論していた。

いくつかの案があったが、大別すると 2 種類ある。
...
暗黙的に `.js` というファイルを JS ファイルと判断しているツールにとっては `.mjs` は単に未知のファイルとなってしまう。
...
シンタックスハイライトなどは、拡張子を認識しても `import` や `export` といった構文の対応や依存解決の表示といった周辺の挙動も合わせて対応が必要になる。
...
仕様策定側も、最終的にはモジュールベースの ESM が基本となっている世界を想定している。

特にブラウザが、複数の JS を同一スコープで連結するといった挙動も、黎明期の JS ではまだしも、十分発展した現在の JS においては不要となりつつある。
...
この場合、キャッシュなどの設定はこれまでの `bundle.js` ファイル 1 つの設定とは別の最適化が行われていくだろう。
