...
メッセージについては、明示的な `read()` もしくはそれを行う Stream の Pipe で行うため、`onmessage` イベントもいらない。
...
UnidirectionalStreamsTransport は以下のような定義だ。
...
一方 BidirectionalStreamsTransport の定義はこうだ。
...
さて、これを何に使うかという点について考える。


### Media Stream

まず、このブログでも以前紹介したように、WebRTC が比較対象に挙がるのであれば、メディアの転送がどうなのかという問題になる。
...
DatagramTransport を使えば out of order / unreliable な read/write が可能なので、現時点ではここが QuicTransport を使用するモチベーションの一つとして考えられるだろう。
