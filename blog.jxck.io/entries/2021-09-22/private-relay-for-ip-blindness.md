...
ここからさらに、HTTP の `Accept` ヘッダや `If-None-Match` を含めたり、JS でさまざまな API を叩いて Canvas や Font などデバイス固有な挙動をするものを探し、精度を上げるために各社がしのぎを削っている。
...
独自の規格/仕様かつオープンな実装ではなく iCloud サービスの一環としてリリースするという、よくも悪くも Apple らしいやり方なので、Apple が突然また過激なことを言い出したと思う人もいるかもしれないが、この流れがわかっていれば、順当以外の何物でもないとも言える。
...
Apple の資料によれば、Private Relay が有効になると、Ingress Proxy(前段) と Egress Proxy(後段) という 2 つの Proxy が割り当てられる。
...
日本の東京にある Proxy からリレーされていることもわかる。
...
Private Relay が公開されて以降、モバイルキャリアなどから、特定のサービスの利用をギガの消費にカウントしないカウントフリーサービスや、フィルタリングサービスなどが正常に機能しなくなる可能性がアナウンスされている。
