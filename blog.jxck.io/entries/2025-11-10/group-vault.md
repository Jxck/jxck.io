...
権限管理の方式は色々あるが、1Password ではユーザを Group に追加し、その Group に権限を付与することで管理するのが基本だ。

いわゆる管理者となるアカウントも、実態は「管理者」Group に入ったユーザという形だ。

デフォルトで作られている Group は以下の 3 つがある。
...
したがって、基本は「管理者」と「メンバー」がいると思えば良い。

Business アカウントを作ったら、まず最初にこの「管理者」Group に複数のアカウントを登録し、SPoF を防ぐのが重要だ。
...
そもそも Owner は Group なので、管理担当者は個々に自分のメールアドレスを用いてアカウントを作り、それを Owner Group に追加するのが 1Password の運用方法になる。

このメリットは、「誰が Owner であるか」の管理が 1Password の Group を見ればわかる点だ。Admin アカウントをアイテムに入れて共有すると、ダッシュボードだけでなく、そのアイテムの共有範囲を別途管理する必要があり、二度手間なだけになる。
...
個々がきちんと管理している、それぞれの Business アカウントを「管理者」Group に追加し、素直に運用するのが、1Password Business の機能を最も有効に使う方法だろう。


### Security Group

もう 1 つ、デフォルトでは作られていない Security という Group がある。

これは、「ドメイン侵害レポート」という機能を有効にすることで作られ、そのレポートを見ることができる権限が付与される Group だ。
...
この機能のみ Group が別で、「管理者」Group に権限を振ることもできないようだ。なぜそうなっているのかはよくわからなかったが、おそらくレポート内容の機微性を踏まえてそうなっているのだろう。


### チームメンバー

デフォルトでは、追加したユーザは全て「チームメンバー」Group に追加される。

組織が小さいうちはそれでも良いだろうが、大きくなってくるといくつかの問題が出るだろう。

- 「管理者」だと権限が強すぎるため、ある程度の権限を持つ「マネージャ」ロールが欲しい
...
今回はチームにマネージャが 1 人なので、Vault に 1 人ずつ追加すればそれでも足りる。

しかし、Frontend/Backend Vault 両方に Manager Group を追加することで、2 人をマネージャとして機能させることもできる。この場合、例えば Bob にも Frontend Vault が見えてしまう。
