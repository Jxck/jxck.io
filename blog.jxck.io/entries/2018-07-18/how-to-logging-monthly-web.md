...
未だに落ち着いたとはいえないが、2017 年 7 月に始めてから 1 年続けたので、結果として現状どうなっているかをログに残す。
...
たまに W3C 自体が特定の問題や動向などについてエントリを出すこともあるが、ここを見ていればそういう情報も入ってくる。
...
そのタスクが実行されているのが design-review というリポジトリで、ここに issue で Review がリクエストされ TAG のメンバーがコメントする。
...
基本は ML で議論されるが、年 1 回集まる TPAC はやはり重要で、ここでの議論は Minutes として各 ML などに投稿される。
...
他にも様々なイベントが催されており、News Blog を見ていれば知ることができる。
...
Stage は 0~4 があり、大まかに以下のような意味を持つ。
...
その Proposal の出現は、先に twitter などでアナウンスされているようなので、気づきにくいものもある。
...
基本は ML の議論と、Draft のアップデート、年 3 回開催される IETF 会議での Minutes を追う。

WG ごとの F2F や Interop もよくあるため、それらも含めると毎回かなりのアップデートがある。
...
せっかくなので、年末は 1 年分の振り返りを簡単におこなってみようと、去年の 12 月は Yearly Web として収録した。
