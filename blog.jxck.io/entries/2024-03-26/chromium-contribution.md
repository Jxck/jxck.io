...
初めて取り組もうとすると、まずどこを見ればいいかわからないところから始まる。
...
選んで Start Review すると初めてリクエストが送られる。これをしないと誰も見てくれない気がする。
...
レビューが付いたら、そこに対して REPLY でコメントを返すか、次の Patchset で直して、直ったら DONE にする。
...
- debugging.md - Chromium Code Search
  ...
  - `--no-sandbox` で sandbox を無効にして実行しないとうまくデバッグできないケースに注意。
...
- stable: 全プラットフォームで有効(通常 stable になったらこのエントリは消される)
...
別で Finch のコントロールを定義する場合は `base::Feature` のインスタンスを作る必要があるが、その辺は Finch のシステムにアクセスできない外部コントリビュータがやることはあまりないだろう。
...
で表示すると、一番下に `Command-line variations` が追加される。よくわからないフォーマットでワンラインにされているが、これを雑に `,` や `*` で改行すると、有効になってる機能が確認できるだろう。
...
これを作るコマンドがあるので、`src/compile_commands.json` を作ってから、起動しなおしたりすると直る。
...
テスト用のドメインとしては `web-platform.test` などが使われる。
...
これは、WPT のテストケースに対して、通らないものは「落ちる結果」を expected ファイルに書いて、実行時に生成される actual ファイルと一致するかどうかを見るような仕組みになっている。
...
まず、普通にファイル (`/example/test.html`) を置けば、それは静的ファイルとしてアクセスできる。
...
以下のボタンから各種レビュー(Privacy, Security, Enterprise, Debuggability, Testing) をリクエストできる。Intent to Ship の前にこのレビューを終わらせる必要があるが、レビューには割と時間がかかるっぽい。
