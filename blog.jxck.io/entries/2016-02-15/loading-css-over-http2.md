...
この内容は、単に Chrome に対する変更だけではなく、 HTTP2 によって変化する最適化手法と、それをもっとも活かすための HTML, CSS の構成についてのヒントがある。
...
しかし、 HTTP/1.1 では、リクエストの数を減らすためにすべての CSS を 1 つ(もしくは少数個)に結合する最適化が主流だった。
...
このため、コンポーネントごとに CSS を分割するのは、キャッシュの容易性を考えてもよい方法といえる。
...
HTML をすべて生成してから順次送るのであれば問題ないが、本来 HTML は先頭から、準備ができた順にコンポーネントを送信することも可能であるにもかかわらず、それができなくなる。
...
この問題に対応するため、次のような JS を用いて CSS を非同期ロードする方法がある。
...
サイズが分かっていれば、コンテンツを埋める枠のサイズをすべて最初に指定することもできるが、次の方法を使えば、どのような viewport でも適切に表示できることができる。


## HTTP2 時代の最適化

HTTP2 では次のように書くことが可能になる。
...
もし、 Header, Article, Footer の CSS が読み込まれていた場合を考えると、次のような状態になる。
...
- 上から順番に表示され "Above the Fold" がどの部分かは気にしないでよい。
- コンポーネント単位で設計し、そのコンポーネントの直前に `<link>` を書けばよい。
- ストリームで考えられる(`<link>` の有無を先に考えなくてよいのでサーバは完成した順に HTML を送信できる)。
...
Flexbox は小さいコンポーネント単位で使用し、全体のレイアウトには CSS grid を使うのがよいだろう。
...
Firefox では、次のように `<script>` をはさむことで CSS がロードされるまでパースをブロックし FOUC を回避できる。
...
すべてをまとめると、現状の Firefox, IE/Edge では Progressive Rendering が可能になる。
現状の Chrome & Safari では CSS がロードされるまで White Screen になるが、従来のようにすべてを `<head>` に置いていたのと比べれば問題ではない。
