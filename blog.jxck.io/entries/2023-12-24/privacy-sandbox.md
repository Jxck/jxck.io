...
多くのユースケースごとに、代替手段となる API を策定し、それを実装し、リリースする。もちろん、シェアの大きいブラウザを保つ Google は、自分たちの事業に有利な API を Ship できる可能性もあるため、そうならないような監視自体も必要だ(この点も先の Final Report にもちろん書かれている)。しかし、3rd Party Cookie 自体のブロックも CMA の許可を得て初めて リリースできるのは、広告事業へのインパクトは Privacy Sandbox の登場よりも、3rd Party Cookie の廃止の方がよっぽどインパクトが大きいと考えられているからなのだ。
