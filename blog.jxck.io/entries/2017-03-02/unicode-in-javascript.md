...
```js
[0x00, 0x01, 0xF6, 0x2D] // 😭
// 0x1F62D を二進数にし 32 bit になるまで先頭に 0 を追加してから 8 bit ずつ区切った配列
```

受け取った側は、データを 32bit ずつ Code Point とみなして文字に置き換えていけば良いし、受け取ったバイト数を 4 で割れば文字の数もわかる。
...
ウムラウトやマクロンのような記号でも同じことが起こる。以下の文字は 3 つの方法で表すことができる。
