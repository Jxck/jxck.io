...
![会場入口にある Google IO のオブジェ](io.png#704x528)

Web はオープンなので、I/O で知らない話が出てくることはほぼない。

そこで AI を中心に回った。

![開発者向け AI デモブース](ai-dev-zone.png#648x486)

全体のハイライトは Gemini だが、開発視点では Antigravity だった。

出た瞬間だけ話題になり、完全に忘れ去られていただろう Antigravity だが、G 社内では使われているという噂は聞いていた。

どの Googler と話しても、今社内でドッグフーディングしながらプロダクション開発をしているようだった。

それでも、ここまで大々的に返り咲くとは、思っていなかった。

![Antigravity CLI Demo](antigravity-cli.png#648x486)

今表に出ている Antigravity は、本命のそれではないらしい。

おそらくそれは、Gemini 3.5 Pro が出ていないからなんだろう。
...
![カリフォルニアの日差しは強い](strong-sun.png#648x486)
...
「こういうものが必要だ」と思って手元で捏ね回しても、ふと顔を上げるとサービスとしてリリースされている時代。

![A fireside chat on the evolution of the developer craft](fireside-chat.png#648x486)
...
- 自前でチップを作れる。最近始めたんじゃなく、ずっとやっていた。
- ゴリゴリにデータセンターを増やしている。US では原発も再加熱している。
- 世界有数の AI エンジニアがいる。そのトップが伝説の Jeff Dean だ。

そういえば、そういう会社だったよなと思い知らされた感じはあった。

![Google Logo](google.png#608x810)

俺達がハーネスを組んでまで AI に、繋ぎたいのは「既存資産」だ。

モデルが知らない、社内にしかない知識を与え、「社員」としてオンボーディングしている

何かを繋ぐたびに、「この情報は、こっちの AI に読ませていいんだっけ?」という折衝が、各地で行われている。
...
![find your way](find-your-way.png#608x810)
...
![Canalun](canalun.png#608x810)

Chromium のログも以前は、Claude で Co-Sign されたコミットをチラホラ見たが、最近は見なくなった気がする。こちらも Antigravity が中心らしい。

Gemini しか使えないのが嫌で辞めた開発者もいた。Sundar の号令もあって、それぐらい使われているんだろう。

Google には、開発者の環境を育てる専門のチームがある。GitHub を使わずに Gerrit や Monorail を自分たちで作って使っているのは、載せたら GitHub が耐えられないからだと聞いた。

世界中に散らばった CI クラスタが落ちる理由の一部が宇宙線だ。宇宙から降り注ぐ線がメモリ上のビットを反転させて、エラーになって CI が落ちる。そんなものがちょいちょい観測される規模で並列実行されている CI を、GitHub Actions なんかに載せられるはずがない。

逆を言うと、それを長年実現しているチームがある。AI だけのチームでも、開発するだけのチームでもなく、「開発できるようにする」ためのチームだ。

そんな社内で Gemini をみんなで育てていたら、まあ育っているんだろうなぁ。

![yuku](yuku.png#608x810)

俺達は、今までさんざんモデル開発競争を見てきた。

昨日はこのモデルのリリースに驚き、今日はこのモデルの性能に湧いてきた。

全部賭けようと思って移行が終わる頃には、もっと良い何かが出てきている。

なのに最近は、たった 1 個のモデルだけに半年ぐらい世界が湧いている。
...
![Chrome チームのパーティー](chrome-party.png#627x472)

SF で WebKit の niwa さんとディナー。WWDC も楽しみ。

![dinner](dinner.png#648x486)

CHM は 2h じゃ回れない。

![The Mosaic Handbook](mosaic-handbook.png#608x810)

Stanford はデカい。

![Stanford Univ](stanford-univ.png#648x486)

SF へ向かう道中も Podcast を収録。

![SF への移動中にドライブトーク収録](drive-talk.png#810x608)
