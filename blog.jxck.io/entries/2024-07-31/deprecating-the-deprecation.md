...
世間では「3rd Party Cookie の Deprecation を撤回した」と騒がれているようだが、そもそものタイトルは「Privacy Sandbox の方針転換」だ。
...
ここで、表示されるプロンプトには 2 種類の方式が考えられる。
...
iPhone アプリはインストール後の最初の起動時に 1 回だが、Web の場合はページに遷移するたびにプロンプトが出て、ユーザにとっては非常に煩わしいことになる。その上で
...
プロンプトの出し方についても、1 回のリリースでは終わらず、ここから先数年かけて徐々に行なっていくことになるんだろう。
