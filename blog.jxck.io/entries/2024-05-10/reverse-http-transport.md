...
Origin をデプロイした瞬間に CDN に事前にコネクションを確立しておく。そして CDN はそのコネクションを逆に使って HTTP を転送するのだ。
...
つまり、ネットワークが分かれない構成では、この固定 IP が漏れると直接 DDoS は可能となるのだ。
...
しかし、Reverse HTTP なら、Origin は起動するたびに CDN に繋ぎにいくだけで良いので、CDN は常に「張られている接続」に対して負荷分散を行えば良い。
...
これは、CDN と Origin がネットワーク的に分かれている構成だと面倒な気がする。

やはり、従来のクラウドでのやり方のように、Origin が Private Network に並んでて CDN がその境界にいる構成の方が組みやすい気がする。ただ、その場合は (EC2 - ELB/ALB みたいに) Reverse する旨みがあまりない気がするが、どうなんだろうか。
...
- Private Relay のような OHTTP Proxy
- 自分で立てるなんらかの用途の串
