...
メールクライアントや RSS リーダーのようなユースケースを PWA で実装する場合、バックグラウンドで定期的にタスクをしたいケースがある。
...
しかし、 Mailer の受信ボックス更新のように、バックグラウンドで定期的にタスクを実行したい場合がある。
...
まず、定期的に SW を起動できため、そこで Crypt Mining などのリソース消費が可能になる。
...
今回の場合は、ユーザの許可なく PBS が登録され、任意の状態(タイミング)でイベントが発火し、任意のタスクが実行できる場合にこうした懸念が発生するため、これらを防ぐための制限が必要だ。
...
まるで PBS という機能自体が悪いかのように書いたが、同様のことは Native App では実現可能だ。
...
そこで、頻繁に使うサイトの PBS の頻度は高く、使ってないサイトでは発火させない、という Intervention を導入することになる。
...
- Periodic Background Sync - Issue #214 - mozilla/standards-positions
