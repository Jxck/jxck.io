...
しかし、たとえば Device Pixel Ratio が大きい Retina 対応端末などには、 大きいサイズのファイルを提供しなければ、拡大表示による画像の荒れが発生してしまう。
...
しかし、 200x200 で表示すれば、同じ正方形を縦横ふたつ並べて大きくしないといけないため、鮮明さが落ちる。
...
スクリーンがもつ 1x1 を描画するための四角は **Device Pixel** といい、これが小ほど **画面密度(Device Pixel Ratio)** が高いと表現する。
...
したがって、 300x300 を用意しておき、相手がだれであろうとそれを送って、 200x200 なり 100x100 なり都合のよいサイズで表示してもらえば、 **見た目の問題** は解決する。
...
Media Query を用いて DPR 値で分岐した場合次のように指定できる。
...
sizes を用いて、画像のサイズを明示した場合次のように指定できる。
...
スクリーンショットを x3 のデバイスで取得できればよいが、現状筆者はそれを持っていない。
