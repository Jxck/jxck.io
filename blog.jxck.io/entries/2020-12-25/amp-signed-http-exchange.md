...
- 通常の HTTPS 用の証明書にはない CanSignHttpExchanges 拡張が必須
...
鍵は openssl などで普通に作れるが、CanSignHttpExchanges 拡張のついた証明書は現状 Digicert しか発行していない。また、発行した証明書は、今回は AMP SXG に使用するが、通常の SXG にも利用できるため、それはまた別途解説する。
...
本ブログでは、躊躇なく Production でこういう作業をゴリゴリ行うので、作業している途中から Bot が来てちょっと邪魔だった。
