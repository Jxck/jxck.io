...
なお、Podcast アプリでの視聴に必要な feed, mp3, artwork を提供するホストについては、Dual Stack を維持しているため、Spotify などでの視聴に影響はない。
...
しかし、その最初のアクセスには平文通信が残り、ブラウザは HTTPS First Mode, Preload HSTS, HTTPS RR などを模索しながら、最初の通信から暗号化する方法を探っている。

本サイトでは、`:443` のみを受けるようにするとともに、`:80` を listen するのをやめ、ufw でも塞ぐことにした。

これにより、そもそも `:80` とは接続を確立できないため、平文でペイロードが露出することはない。
...
HTTP が暗号化されても、その手前の DNS Query が平文だと、「mozaic.fm を名前解決した」という事実がネットワーク上に露出してしまう。

そこで DNS over HTTPS を用いると、DNS Query を HTTPS で暗号化することができる。

これについては、筆者が対応するのではなく、DoH に対応した DNS をユーザに使ってもらう必要がある。
...
ところが、DoH で引いた値が Cache Poisoning されていれば、偽のレコードで攻撃者サーバに接続させられる可能性は残る。

そこで、筆者は DNSSEC を有効にし、レコードに署名をつけている。

これによって、署名のチェーンを辿ればレコードが改ざんされていることを検知できるため、安心して名前解決して欲しい。
...
以前、登録したドメインの証明書が CT Log に追加されると、Facebook にメッセージが来るなぞの連携があったが、それはどうやらサ終してしまった。
...
要するに、誤発行された証明書を失効させるより、そもそもすぐに期限切れで無効になれば良いという考え方だ。
...
もともと Short Lived にする前は、Certbot を systemd timer で 48 時間おきに起動していた。しかし、Short Lived の場合、Certbot は残り 80 時間を切ると更新をかける。もし 80 時間を切ったあとの起動で Certbot が失敗したら、2 日後の timer 発火時には証明書がすでに切れている可能性がある。そこで毎日起動するように直し、残りの有効期間を毎日チェックしながら更新するようにしている。
...
ところが TLS/1.2 には歴史的な蓄積が大きく、現在となっては非推奨な方式もサポートされており、設定に気をつけるしかなかった。
...
TLS/1.3 では、実装必須の Suite は `TLS_AES_128_GCM_SHA256` と定義されている。
...
ところが、Grover のアルゴリズムにより、量子コンピュータを用いると鍵探索コストを削減できると指摘されている。AES-128 は概ね 2^64 相当に落ちるというものだ。
...
要するに、NSA CNSA のような国家の調達基準など、別に定める Standard Profile によってこれを上書きする余地を残しているのだ。疎通ができないリスクを飲んだ上で、AES-128 を落とし SHOULD である AES-256 や ChaCha20-Poly1305 のみをサポートする要件が定義されることもある。
...
これらについては、「ML-KEM がもし破られた場合、単体ではフォールバックがない」という観点から使われている。これは、ML-KEM の実績が少ない現時点で、単体でも安全かどうかの議論が続いているためだ。

現状、多くのブラウザは `X25519MLKEM768` を採用し、純粋な ML-KEM が実装されるのは先になりそうだ。そこで、ハイブリッド方式と純粋な ML-KEM を両方サポートしておくことにした。
...
TCP を閉じた wiki.mozaic.fm は、基本的に QUIC しか通らない。すると H/2 と H/1.1 で疎通できる余地はない。
...
特に Client Hello にある SNI を見れば、どのドメインに接続しているのかはわかってしまう。
...
mozaic.fm は、Web 技術について議論する Podcast であり、最新の技術動向を常に注視している。

ただ、注視するだけでなく、それを実践する場として https://mozaic.fm を使っている。今回の刷新では、これまで試せていなかった様々な技術を試すことができた。

こうした実験場所を持っていると、プロダクションでは決してできないデプロイを試し、「理論上はあり得るが、実践したことがない」構成を実践できる。

本当は、この刷新で Cloudflare Worker に載せようと思っていたが、こうした実験をするには Cloudflare Worker では無理だったため、採用を見送った。

結果、この mozaic.fm は、おそらく世界でも有数の尖ったデプロイとなり、それゆえに接続が非常に難しくなった。

しかし、Podcast は Podcast アプリで聞く人がほとんどなので、Podcast アプリが必要とする feed, mp3, artwork については、従来通り配信しており、視聴には問題ない。
