...
理想だけを言うなら、Passkey をサポートし、Username-Less のログインを可能にする。その上で、パスワード+TOTP の認証はサポートを止め、保持していてもリスクでしかないパスワードのハッシュを、サーバから消す。鍵の管理とリカバリにだけ注力すれば良い。
...
ユーザの遷移に挟んでまで、これをしっかりとアドバタイズしているサイトは、パッとは思いつかなかった。
...
![Watchtower で未移行のアイテムが表示される](watchtower.png#516x512)
...
とにかく、サービスがログイン画面や管理画面に Passkey の導線を入れて、あとはユーザの選択なので自己責任にお任せです、では正直「サポートした」の半分にも達していないと筆者は感じる。
