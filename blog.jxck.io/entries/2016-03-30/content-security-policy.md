...
ポリシーは、コンテンツが読み込み可能なコンテンツについての制限であり、次のような指定が可能である。
...
基本的には、同一オリジン以外の外部スクリプトや、 `<script>` 内に直接記述するインラインスクリプトなどをすべて除外し、そうしたスクリプトがあった場合も実行を阻止する。
...
まず、この指定により、インラインスクリプトがすべて無効になる。
よくある例として、 Google Analytics のためページ下部に埋め込んだ `<script>` がすべて動かなくなるため、アナリティクスが動かなくなる。

他にも、たとえば `cdn.jquery.com` などの Public CDN から取得しているスクリプトも軒並み動かなくなる。
...
次のように、ポリシーに `report-uri` ディレクティブでレポート先 uri を指定する。
...
本サイトのコンテンツは、すべて筆者の管理下にあるため、影響の把握はそこまで難しくはない。
...
一番懸念しているのは、たとえば本サイト購読者の、ブラウザ拡張やブックマークレットなどへの影響である。
...
ひととおりエラーを見て回ったところ、次の点で修正が必要だった。
...
よって、通常のページと AMP 対応ページでは次の出し分けをしている。
