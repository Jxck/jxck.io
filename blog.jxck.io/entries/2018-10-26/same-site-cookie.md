...
サーバは、Cookie の正当性を確認し、正しければ投稿を受け付ける。
...
罠サイトは Referrer Policy を用いることで、Referer を送らないように設定できるため、Referer をベースとした対策は万全とは言い難い。
...
しかしこの場合も、罠サイトからのリクエストに Cookie が送信されなければ、毎回ログインページのレスポンスが返ることになり攻撃が成立しない。
...
しかし、代表的な攻撃シナリオはやはり、ターゲットを罠サイトなどに誘導し、微妙にペイロードを操作しながら連続でリクエストを発生させ、自動で付与される Cookie を推測するというものが多い。
...
POST では Cookie が送られないため、前述した CSRF のような攻撃への耐性が高まる。
...
実際に Same Site Cookie を導入する方法は、大きく 2 つある。
...
Read Cookie
: ユーザのセッションを維持するための Cookie 。これが送られればユーザがログイン状態と見なされる。しかし、それ以上の操作についてはこの Cookie では許可されない。
Write Cookie
...
もし新規の開発や改修が可能であるならば、こうした構成も可能かもしれないが、既存のサービスのほとんどはここまで手を入れるのは難しいだろう。
