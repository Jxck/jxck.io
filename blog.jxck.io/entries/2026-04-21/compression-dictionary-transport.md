...
例えば HTML の場合、`<!doctype html>` で始まる可能性が非常に高く、画像などのバイナリデータは、ヘッダ部分には共通するパターンが多くなる。それらは、1 回しか出てこないことも多い。
...
この辞書は、レスポンス HTML の多くの箇所をカバーするため、全体の圧縮効率を向上することができる。
...
前後は v0 の該当部分を参照し、変更部分のリテラルだけがある状態だ。おかげで、440Byte が 19byte まで圧縮できている。
...
辞書の有効期限は、単純に辞書の `Cache-Control` で行う。
...
ただし、本サイトでは実装を小さくするため、このすべては実装してない。例えば、他のサイトから Origin をまたいで遷移してくる場合は、辞書の圧縮を行ってない。対応しなければ、従来通り `.html` や `.html.br` にフォールバックするだけだ。
...
brotli の圧縮内容を確認する brotlidump.py というツールもあるが、こちらは `-D` による独自辞書指定に対応してない。AI がパッチを当てて動かしてくれたので助かった。
