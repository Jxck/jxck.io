...
従って、bundle された `main.js` の 1 箇所が修正されただけでも、更新されていない依存を含めた bundle.js 全体を再取得する必要があるのだ。これは非常に無駄が多い。
...
ただし、本サイトでは実装を小さくするため、このすべては実装していない。例えば、他のサイトから Origin をまたいで遷移してくる場合は、辞書の圧縮を行っていない。対応しなければ、従来通り `.html` や `.html.br` にフォールバックするだけだ。
...
Compression Dictionary Transport の検証のため、本サイトの HTML を事前辞書で圧縮し、検証を行った。
