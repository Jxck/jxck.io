...
例えば HTML の場合、`<!doctype html>` で始まる可能性が非常に高い。構造化されたフォーマットほど、ヘッダ部分には共通するパターンが多くなるのだ。しかし、それらは 1 回しか出てこないことも多い。

そこで、あらかじめ `<!doctype html>` を参照用に用意しておき、それを圧縮対象の手前にくっつけてから圧縮を始めると、こうしたパターンも圧縮できることになる。この方法は、バイナリでも通用する。

このとき、圧縮で参照するために頻出パターンを集めたファイルを、LZ77 では辞書(Dictionary)と呼ぶ。汎用アルゴリズムを超えた圧縮を実現するには、いかに「精度の良い辞書」を用意するかが、圧縮性能を大きく左右することになる。
...
Brotli が RFC になったのは 2016 年だ。すでに 10 年経ち、その間にも Web には様々な変化がある。新しく増えた API や、頻度が増えた構文パターンなどは、既存の辞書には含まれていない。その変化を辞書に反映するには RFC を更新する必要があるのだ。

また、あくまで世界中のトラフィックを集計して作った汎用辞書であるため、自分が配布するコンテンツに特化したチューニングなどもできない。
...
例えば、テンプレートエンジンにコンテンツを流し込んでいるサイトの場合、テンプレートはどの記事にも現れる共通部分と捉えることができる。そのテンプレートを連結したものは、HTML を圧縮する際の辞書として使えるだろう。雑に言うとこうだ。
...
この辞書は、レスポンス HTML の多くの箇所をカバーするため、全体の圧縮効率を向上させることができる。


### 差分辞書

まず、Lorem Ipsum の冒頭を v0.txt として保存する。

```text:v0.txt
```

次に、途中の 1 単語だけ変更した v1.txt を用意する。

```diff
- Duis aute irure dolor in reprehenderit in voluptate...
+ Duis aute irure dolor in Jxck in voluptate...
```

このとき、v0.txt を辞書に v1.txt を圧縮してみる。

```console
$ brotli --dictionary=v0.txt v1.txt
```

この結果は、以下のようなバイナリになっている。

```
======== Meta block contents ========
1. COPY   259 bytes from v0.txt[0:259]
2. INSERT "Jxck"
3. COPY   177 bytes from v0.txt[272:449]
```

前後は v0 の該当部分を参照し、変更部分のリテラルだけがある状態だ。おかげで、440 byte が 19 byte まで圧縮できている。

つまり、*両者の Diff だけを送っている* ような状態にかなり近いといえるのだ。

これを JS などに応用すると、bundle.v0.js を辞書にすれば、その一部を更新してビルドし直した bundle.v1.js は v0 と v1 の差分だけに圧縮できるのだ。

このように、辞書を用いた差分の圧縮は、**Web における差分更新** に応用できるのだ。
...
差分更新は、現代ではスタンダードな JS の Bundling でも効果が期待された。

我々は ESM を手に入れてからも、Native な `import` はほとんど使っていない。`import` 段階で、多くの依存を取得すると、オーバーヘッドが大きいため WebPack などで 1 ファイルにまとめて配布している。その中では Native な `import` は使っていない。`import` は Bundler が見るための構文としてしか使われていない現状があるのだ。

したがって、bundle された `main.js` の 1 箇所が修正されただけでも、更新されていない依存を含めた bundle.js 全体を再取得する必要があるのだ。これは非常に無駄が多い。

そこで複数の JS を Bundle して bundle.js を作るのではなく、ばらばらのまま WebBundle で一括配布する。クライアントは、ばらばらに Fetch されたかのように import できる。そうすれば、JS の `import` 時に Fetch するのと異なり、ネットワークオーバーヘッドを減らして、生の `import` が使える。個別のキャッシュもできる。これが WebBundle に期待されたメリットだ。
...
差分更新ができなければ、結局最新の WebBundle ファイルを毎回取得する必要があり、bundle.js と差がないことになる。そして、徐々に作業がなくなり、実質終わったコンセプトと言って良い状態になった。

その後始まった Shared Brotli には、この「差分更新」のコンセプトを、辞書を用いて再始動するモチベーションがあったのだ。
...
全ページで、そのページ用の woff2 を作っておく事前作業や、それを「クライアントにすでにある woff2」を辞書として圧縮するオーバーヘッドは残る。そもそも、1 ファイルの転送は残るため、ネットワークが遅ければフォント置換による FO(I/U)T は避けられないだろう。それでも、CJK で WebFont をどうしても使いたい開発者にとっては、縋れる藁ではあるだろう。
...
しかし、この仕様は提案時期が 2008 年頃とかなり古く、仕様も複雑だった。当時はまだ CORS なども普及する前であるため、安全性の問題もあり、全くと言ってよいほど普及せず、2016 年には Chrome から Unship の Intent が出た。

- Intent to Unship: SDCH
  - https://groups.google.com/a/chromium.org/g/blink-dev/c/nQl0ORHy7sw/m/LkXwXcUtAgAJ
...
CDT では、任意のコンテンツから辞書を作成し、それをサーバ/クライアント間で共有する 2 つの方式が提案されている。
...
すでに取得したリソースを、次に送信するリソースの辞書として使う方式だ。
...
しかし、動的に HTML を生成し、レスポンスしたら捨ててしまうタイプの実装では、ハッシュから辞書となる HTML を引けないため、CDN などに HTML をキャッシュしているような構成の方が相性が良い。辞書にするためメモリに持つ場合は、その管理の戦略も必要だろう。
...
どのような辞書なのかは `Use-As-Dictionary` の `type` で指定することになっているが、現状はデフォルトの `raw` しかない。`raw` はいわゆる "unformatted blob of bytes" として仕様に定義されているため、`application/octet-stream` を指定するのが妥当そうだ。
...
作成した辞書は `/dictionary/entries/<sha256>.dict` として配布している。
...
全ての `.html` の隣には、従来通りの `.html.br` と、新しくビルドした `.html.dcb` ファイルがある状態だ。
...
`.br` と `.dcb` で辞書の効果を比べると以下だ。

Caption: `.br` と `.dcb` の比較
| 指標                     | 値         |
|:------------------------:|:----------:|
| `.html.br` の平均サイズ  | 6,486 byte |
| `.html.dcb` の平均サイズ | 3,708 byte |
| 平均サイズ比             | 53.20%     |
| 平均削減率               | 46.80%     |
...
作成した辞書は、`blog.jxck.io` 以下の全てのパスで、`Link` ヘッダでアドバタイズしている。
...
辞書の有効期限は、単純に辞書の `Cache-Control` で管理する。
...
ただし、本サイトでは実装を小さくするため、この全ては実装していない。例えば、他のサイトから Origin をまたいで遷移してくる場合は、辞書の圧縮を行っていない。対応しなければ、従来通り `.html` や `.html.br` にフォールバックするだけだ。
...
brotli の圧縮内容を確認する brotlidump.py というツールもあるが、こちらは `-D` による独自辞書指定に対応していない。AI がパッチを当てて動かしてくれたので助かった。

- brotli/research/brotlidump.py at master · google/brotli
  - https://github.com/google/brotli/blob/master/research/brotlidump.py

zstd の方は、`--train` という辞書を作るモードが備わっている。しかし、この辞書はそのままでは CDT には使えない。これはかなりハマって、結構な時間を溶かしたので、次回解説する。

プロトコル自体がもう少し使われるようになれば、開発の周辺環境も改善されるだろう。
...
![CDT により辞書で圧縮された HTML とそのヘッダ](./dictionary-compressed-html.png#901x709)


## Outro

Compression Dictionary Transport の検証のため、本サイトの HTML を事前辞書で圧縮する対応を行った。
...
- Mozilla Standard Position
  - Compression dictionary transport · Issue #771 · mozilla/standards-positions
