...
このとき、submit されたデータの収集方法はいくつかある。
...
また、XHR や fetch を使ってそのまま POST することができる。
...
しかし、File でもない限り HTML Form からは `application/x-www-form-urlencoded` で送られてくるという前提で実装されたものもあるだろう。
...
また、そのまま POST の Body にすれば、`application/x-www-form-urlencoded` として送ることができる。
...
ここでは FormData が iterable であることを利用してオブジェクトを組み立ててみる。
