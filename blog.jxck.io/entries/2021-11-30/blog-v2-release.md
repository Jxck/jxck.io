...
`<table>` 用の CSS と `<pre>` のシンタックスハイライトは、他の要素に比べて CSS の量が多いのにもかかわらず、出現頻度が低い。

そこで、`<table>` と `<pre>` の CSS はファイルを分け、HTML に出現する直前に CSS を 1 度だけ差し込むようにしている。
...
ここまでに挙げたような本サイトに必要な機能は、1 つに盛り込んでも良かったが、最終的にはブラウザでの挙動も想定して作っておきたかった。
...
鉄下駄として Web Font を入れていたが、もう試すことはだいたい試した上に、もう日本語フォントにこれ以上期待するのは難しいと考えてやめた。
...
- これまでの HTML の細かな問題、気に入らなかったところも全て清算できた
