...
基本は `Domain=example.co.jp` のようにドットが 2 つ入るが、以下の特別な 7 つの TLD のみ `Domain=example.com` のようにドットが 1 つで良かった。今のように TLD が乱立し eTLD を判定しないでも良かった からできたロジックだ。
...
Lou のブログの方では、1996 年ごろから Double Click などが行っていた 3rd Party Cookie による広告のトラッキングが、すでに問題になっていたことが触れられている。ここで Lou は 3rd Party Cookie をブロックするかどうかの決断を委ねられ、数週間悩んだ結果、以下の理由で残すことにしたとしている。(意訳)
