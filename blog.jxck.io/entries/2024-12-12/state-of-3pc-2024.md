...
この方針転換で最も重要な「ユーザの選択 UI」は、プロンプトであるだろうことが発表されているため、であれば実装で注目すべき点は「出るタイミング」と「影響範囲」の 2 点だろう。
...
前者であれば、既に Push がほぼブロックされていた実績から、3PC も同等にブロックされることが予想できる。そうであれば、Chrome は Deprecate しなかったが、業界的にはほぼ精度が出ず Deprecate されたも同然。という状態があり得るため、あとは「サービスプロバイダ」と「サービスユーザ」という構図で、サポートをどうするかといった話に落とし込まれる。
...
憶測で極端な批判をするには、現時点ではあまりにも材料が足らないことだけは、はっきりとわかるだろう。
...
2025 年に結論が出るのかも怪しいが、引き続き注視していきたい。というか、いかざるを得なくなってしまった。