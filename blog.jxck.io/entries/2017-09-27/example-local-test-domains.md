...
もっと言えば、gTLD (generic top level domain) ですら近年は色々と増えている。
...
以上のことを踏まえれば、将来に渡って取得されないことが保証され、こうした用途に使えるドメインがあるのが望ましい。
...
TLD としての `*.example` 以外に、以下の 3 つの SLD も予約されており、同様に使うことができる。
