# [ai] 認知がサチる(Cognitive Saturation)
...
Google の統計サイトで見ると、やっと半分まで来たところだ。

![世界の通信の半分が IPv6 になっている](./ipv6-adoption-world.png#635x413)

![日本の通信も 6 割近く IPv6 になっている](./ipv6-adoption-japan.png#526x336)

https://www.google.com/intl/en/ipv6/statistics.html#tab=ipv6-adoption

IPv4 の割り当てが枯渇しても、余剰アドレスは中古市場で取引されている。アドレスが少なくとも、NAT を始めとした延命技術がなまじ成果を出してしまったため、未だに IPv4 の運用は珍しくない。

HTTP/3 のせいで NAT Table が溢れるといった問題を QUIC のせいにし、UDP を閉じるかなどと騒いでまで、NAT にしがみつく人もいる。これからのインターネットの基盤を策定する上で、IPv4 を前提にするはずがないにも関わらずだ。

一方、Apple のストアは v6 対応が必須となり、大手クラウドでの v4 は課金対象かつその料金は上がっていくだろう。米中を始めとする多くの国が、政府/行政システムの v6 移行を強く進めている。
...
マンションの場合、契約の見直しや、設定の確認だけで有効になる場合もあるかもしれない。(場合によっては「夜ネットが遅い」問題も解決するかもしれない)。

社内の場合、「うちの会社は未だに v4 なのか」と気づくと、イントラに対する見え方も変わるだろう。管理者に問い合わせて、高度に洗練された七色の言い訳を聞いてみる機会になればいい。

クローラや RSS リーダーを使ったり、自分で作っているユーザは、もし挙動が壊れたら、実装を見直すきっかけになる。
...
そこで、IP レイヤでの Dual Stack は維持しつつ、アプリケーションレイヤでエラーとすることにした。
...
かなり汎用的にリクエストのエラーを示すことができるが、主に「HTTP リクエスト」自体に問題がある場合に返す想定だ。しかし、今回はリクエストのシンタックスやフォーマットには問題がないにも関わらず返すことにはなる。
...
しかし、ここで言っている「プロトコル」は基本的に HTTP/1.1 のことであり、それを WebSocket なり別のプロトコルに変えるための要求に使う。

また、このレスポンスには「このプロトコルにアップグレードせよ」を示す `Upgrade` ヘッダが MUST であり、現状 IANA に登録されたプロトコルは以下だけだ。
