...
画像においてもっとも無駄なのは、サイズの大きい画像を小さく表示している場合である。
...
一点の色の情報を 24bit で表しているものを、たとえば 8bit に変換することで、データ量を減らすことができる。
...
JPEG は、大きい画像を送信する際に、ベースとなるデータを先に送り、差分となるデータをあとから追加して画像を完成させることができる。方式はふたつある。
...
あらゆる画像をふたつの形式で保存した場合、サイズがそれぞれどうなるかを検証している。
結果だけ引用する。
...
すべての挙動をなるべく近づけるには、サイズに限らずベースラインに統一するのが良さそうと判断した。
...
画像フォーマットや、最適化対象によって、さまざまなツールがあるが、それらをひとつにまとめた `gulp-image` というツールがあるため、これを採用することにした。

リサイズは別途行い、その結果に対して次のような gulp タスクを作成した。
