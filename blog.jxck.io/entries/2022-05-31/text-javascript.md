...
長いこと行われていた JavaScript の MIME タイプについての作業が完了し、RFC 9239 として公開された。
...
この RFC は、「様々な MIME Type が適当に使われているが、その中できちんと推奨を決めて IANA に登録しよう」という趣旨のものだ。
...
この情報を提供するために、Node や Deno は `.mjs` をはじめとした拡張子や package.json などをサポートし、ブラウザの場合は `<script type=module>` によってそれを知らせることになった。
