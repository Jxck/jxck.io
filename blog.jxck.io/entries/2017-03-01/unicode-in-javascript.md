# [unicode][javascript] JavaScript に置ける文字コードと「文字数」の数え方

## Intro

Input などに入力された文字数を、 JS 数えたい場合はよくある。
多言語対応なども踏まえた上で、どのように処理するのが正しいのか。
`.length` を数えるだけではダメな理由は、文字コードや JS の内部表現の話を理解する必要がある。


それ自体は枯れた話題ではあるが、近年 ECMAScript に追加された機能などを交えて解説する。

なお、文字コードの仕組み詳解自体が目的では無いため、 BMP, UCS-2, Endian, 歴史的な順序など、この手の話題につき物な話の一部は省き、そうした話はものの本に譲ることとする。
...
ところが、 `𠮷` (0x20BB7), `𩸽` (0x29E3D), `𠮟` (0x20B9F), `😭` (0x1F62D) の 4 文字は、 0x10000 より大きいため 2byte では収まらない。
そこで、 UTF-16 では、こうした 2byte では収まらない文字については、倍の 32bit で表す。そして、この 16bit x2 で表される文字を、 **サロゲートペア** と呼ぶ。
...
逆を言えば、サロゲートペアになるのは Code Point が大きい、 Unicode に後から追加された文字が多い。 `𩸽` は後から追加された文字であり、 `𠮷` は `吉` の、 `𠮟` は `𠮟` の **異体字** と呼ばれるものだ。 顔文字も最近追加されたため Code Point が大きい。
...
(逆を言えば、 16bit で収まる文字の範囲、例えば英数字のであると **保証** できるならば length を使うこともできなくはない)
...
[obtain-unicode](https://github.com/Jxck/obtain-unicode)
...
これは、 👨‍👩‍👧‍👦 という顔文字自体が、👨, 👩, 👧, 👦 という 4 つの顔文字とそれを結合する制御文字でできているからである。
