...
これまでは、こうしたことを防ぐには、CA 自身が努力するしかなかった。
...
発行された証明書が必ず登録されていることや、その Log が改ざんされていないことは、どう保証するのかは後述する。
...
CT Log は Merkle Hash Tree を用いているため、Log を改ざんすると不整合が発生する。
...
誤発行に成功した証明書に置き換えられた場合、Chrome での EV のように SCT のチェックが必須でない場合は、正しい証明書として通ってしまう。
...
仕組みはシンプルで、証明書の Hash を Leaf とし、Leaf/Node ペアの Hash を親とする二分木を構築する。
...
CT Log Server には多くの登録依頼が届き、定期的に既存の Tree に追加登録を行う。
...
こうして、CT Log は Merkle Hash Tree を用いて *追記* しか出来ないことが暗号的に保証されている。
...
(また、普通「電子透かし」は Digital Watermarking で、Transparency とは言わないかと思う)
