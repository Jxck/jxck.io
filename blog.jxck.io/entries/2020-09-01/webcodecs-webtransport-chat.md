...
- IPv4 = 1280(v4 MTU) - 20 (v4 header) - 8 (udp header) = 1252byte
...
そうした目的からあえて `<video>` ではなく `<canvas>` を選ぶサービスもあったくらいなので、このメリットはデカイだろう。
...
単純なビデオ会議を実現する程度なら、軽量なフレームワークをサクッと作ることは可能だし、おそらくそうしたものは多く出てくるだろう。どうしても足りない部分はまた別途標準化もあるかもしれない。
...
バイナリをエンコードし、それを送る、その間に必要なロジックは、標準ではなく要件に合わせて開発者が用意するという世界観は、Extensible Web Manifest 以来進められてきた API の低レイヤ化の流れを汲んでいるといえる。
