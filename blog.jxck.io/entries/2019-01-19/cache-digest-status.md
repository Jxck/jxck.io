...
- mozilla standard position に RFP を上げたところ「実装はしないが仕様については *non-harmful*」となりそうだ。
  - [RFP: Cache Digest Issue #131 mozilla/standards-positions](https://github.com/mozilla/standards-positions/issues/131)
...
そこで、Cuckoo Filter という Bloom Filter に似た確率的データ構造を用いるのがこの提案だ。
...
実際、主なブラウザにおいて積極的な実装のシグナルは確認できておらず、仕様は宙に浮いた状態だったという印象だ。
...
もう 1 つ、ちょうど先月提案されたのが Prefer-Push という仕組みだ。
...
他のプロトコルなどのためのスタックとしての用途は残るかもしれないが、h2 が最初にユースケースで挙げていた話では使いにくくなる。
