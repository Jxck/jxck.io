...
特に入れることによるデメリットもなさそうなので、本サイトには `@supports` で CSS を切り替えつつ、試験的に適用することにした。
...
すると、`<h1>` ~ `<h4>` の高さは、フォントが `line-height-step` の値(つまり `24px`) に収まる場合は `24px` に、収まらなければ 48, 72... px に自動でアラインされる。
...
まず `line-height: 1.5` とし、これを基準に `<article>` 全体に Rhythm を当てる。
