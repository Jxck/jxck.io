...
ID で遷移した場合は `:target` 疑似クラスからスタイルが設定できる。

`text=` で遷移した場合も同じことが検討されていたが、現実装では疑似クラスは付かないようだ。
...
これまでの URL の使われ方、ひいては Web の使われ方に少なからず影響するだろうと考えられるので、どのようにデプロイし、どのように広まり、どのように使われていくか、注目したい。