...
Edge が EdgeHTML を辞めた時にも問題になったが、やはり気になるのは多様性だ。

iPhone に Safari 以外のブラウザをわざわざインストールするユーザがどのくらいいるかによるが、もし iPhone でも Chrome をインストールしてデフォルトブラウザにすることが一般的になれば、 Safari シェアを支える最後の砦が崩れることにも繋がり得る。
...
近年の Safari のアップデートには、それまで難色を示していたような機能が続々と追加されていたが、これもそうしたギャップをなるべく埋めた上でこの日を迎えたかったのでは無いかという邪推もできる。しかし、 API によっては根本的に相対しているため、 Standard Position で Safari には絶対に入らないことが決定済みな API もあるため、これを Chromium が提供できるとなると、その格差はスタンスよりも星取表で評価されてしまう可能性もあるのだ。

サービス開発者にとって、 Safari の評判はそこまで良かったとは言えないため、もし Chromium ベースだけに向けた実装ができるなら、そんなに楽なことはないと思う人も多いだろう。しかし、それによってシェアが減ってしまうと、ブラウザ間のパワーバランスは一気に崩れるだろう。
