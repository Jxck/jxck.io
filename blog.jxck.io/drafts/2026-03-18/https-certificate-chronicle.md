...
`http://` が `https://` になった裏で何がおこり、これからどうなっていくのか。まとめていく。
...
当時の証明書発行は、担当者同士がメールベースでやりとりするのが主流だ。企業の担当者は手元で鍵を生成し、対応するリクエストファイル (CSR: Certificate Signing Request) をメールに添付して送る。CA 担当者は、署名した証明書をメールで送り返す運用だ。
...
日本は HTTPS 化の流れに少し出遅れた。

2016 年に筆者が GDE になった時に面接があったが、実態は知り合いのエンジニアとの雑談だった。そこで「日本の HTTPS の普及が凄く遅いんだけどなんでだと思う?」という議論をしたのを覚えている。

![2016 年の段階で日本の HTTPS 移行が他国より遅れている](https-over-chrome.png)

- ウェブ上での HTTPS 暗号化 - Google 透明性レポート
  - https://transparencyreport.google.com/https/overview

要因としては、広告や UGC の Mixed Contents 問題はでかかったが、それは日本固有とはいいがたかった。おそらく「ガラケー対応」が残っているという事情もあるといった議論をしたと思う。

2017 年頃日本のサービスも、Search Rank への反映や、HTTP as non-secure の流れを組み追いかけるように移行を進めた。
...
また、クライアントがプロキシに阻まれたり、OCSP サーバが落ちてる場合、サイトが速くても検証でパフォーマンスが落ちたり、最悪繋がらないこともある。結果、ブラウザは「クエリが正常にすぐ返れば見るが、落ちてもエラーにはしない」という消極的な検証に留めている。要するに、有っても無くても良いということだ。
...
仮に Let's Encrypt が 6 日落ちれば、そのサイトは時間がくれば止まる。証明書の更新失敗に気づき、急いで代替を探し、発行プロセスを移行し、期限前に再発行仕切るというリカバリを、4,5 日で完遂する必要があるかもしれない。

90 日や、せめて 45 日あればまだいいが、6 日となると、先に落ちた場合を想定してフォールバック先を決め、備えておきたい。
