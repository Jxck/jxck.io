...
かつては SSL 証明書として、メールでやり取りし、有料で購入するのが常識だったが、今では TLS 証明書と呼ばれ、プログラムから自動で発行し、無料でも取得できる。

かつては 5~10 年あった有効期限も、どんどん短くなっている。

HPKP は廃れ CT での監視体制が普及し、 CRL や OCSP での失効は証明書を 6 日まで短命にすることで回避する方向だ。

こうした HTTPS を取り巻く変遷は目覚ましく、それは Web を取り巻く環境の劇的な変化を色濃く反映した結果と言える。

`http://` が `https://` になった裏で何がおこり、これからどうなっていくのか。解説する。

(極力ソースを付記するが、既に消えて WebArchive にも残ってないものも多く、筆者の記憶に頼る情報も多い。)

## 黎明期 (90 年代後半~2010 年頃)

90 年代の終わり頃、当時は TLS の前身にあたる SSL というプロトコルでデプロイされていたため、証明書は「**SSL 証明書**」として販売されていた。(いまでもその呼び名は商習慣として[残っている](https://ssl.sakura.ad.jp/))。
...
当時の証明書発行は、担当者同士がメールベースでやりとりするのが主流だ。

企業の担当者は手元で鍵を生成し、対応するリクエストファイル (CSR: Certificate Signing Request) をメールに添付して送る。CA 担当者は、署名した証明書をメールで送り返す運用だ。
...
2000 年前半頃から 無線 LAN、今で言う WiFi が普及し始めた。2008 年に iPhone が登場したあたりからは、お店で配ってる AP をもらってきて自宅に設置したり、会社/図書館/空港/学校などに公開 AP が設置されている環境も徐々に増えていった。
...
経路の盗聴は、有線接続時代では物理的な侵入なども必要になったりとハードルは高かったが、WiFi となると話は別だ。同一 AP に繋げば他人の通信のキャプチャは容易だし、自分のモバイル AP の SSID を "Airport Free WiFi" などとして空港にいれば、勝手にユーザの方が繋いできてくれる。
...
Let's Encrypt も、最初は理解されておらず「無料の証明書なんか怪しい」や「ビジネスで使えるものではない」といった風評もあったように思う。また、当時はまだ EV 証明書も使われており、DV 証明書のみの発行であったことなどが敬遠されたり、ワイルドカード証明書に対応していなといったデプロイ面の課題もあった。

Let's Encrypt の白眉だった点は、構想の段階から 90 日という短命の証明書と、それを自動で更新する ACME プロトコルの策定を前提にしていたところだ。証明書発行の自動化自体は、業界自体が何度が取り組んできたが、ここで初めて今現在多くのユーザが使うプロトコルが誕生する。
...
まず CA は、証明書を発行したらそれを CT Log という分散台帳サーバに記録する。記録時に発行される SCT (Signed Certificate Timestamp) を付与した証明書でなければ、ブラウザは接続をエラーにするという仕組みだ。

SCT が必須になるため、 CT Log への登録が必須となる。つまり、あらゆる証明書の発行履歴が CT Log に残るため、これを監視すれば「自分が発行した覚えのないエントリが、 CT Log に追加された」という事実で、CA の偽装発行を検知できるとう仕組みだ。
...
あまりにも使いづらいため、派生仕様がいろいろと作られることになり,
「影響が大きいと判断したもの」だけを集めた CRLSet(Chrome) や CRLite(Mozilla) を用いたりした。
...
また、クライアントがプロキシに阻まれたり、 OCSP サーバが落ちてる場合、サイトが速くても検証でパフォーマンスが落ちたり、最悪繋がらないこともある。結果、ブラウザは「クエリが正常にすぐ返れば見るが、落ちてもエラーにはしない」という消極的な検証に留めている。
...
CA の責務としては、失効した証明書はすぐさま発覚するようにしたい。そのために行う運用で、パフォーマンスや接続に影響がでては、誰も HTTPS を使わなくなってしまう。
...
なんで Let's Encrypt は証明書を 6 日に短くしようとしているのか、という話について書こうとしたら、中途半端に色々書きすぎてしまった。

こうなってくると書いてないことが目立ってくる

- Google が HTTPS をサーチランクに反映した話
- HTTP でのログインフォームに警告を出した話
- それに呼応して日本では 2017 年頃大々的に移行が起こった話
- hatena のような CGM は mixed contents で苦戦した話
- Upgrade Insecure Request / HSTS
- 署名書を検証しない日和見暗号でつなごうとした話
- 日本はガラケーのせいで世界でも移行が遅れ気味だった話
- EV 証明書でも組織が表示されなくなっていった話
- 鍵アイコンも消えていった話
- Let's Encrypt の軽微なインシデントがあったが、失効を諦めた話。
- etc etc etc

せっかくだから、アドカレにでもするかもしれない。


## DEMO

本サイトを Let's Encrypt の 6 日間の短命証明書に移行した。(それが言いたかっただけだった。)