...
しかし、一旦このファイルが壊れれば、多くのサイトで Cookie に関するなんらかの問題が発生するか、各ベンダがフォークして互換性の問題が発生するか、もっと最悪な何かになるだけだろう。
...
もしここで、「3rd Party Cookie が今までのように使えなくなるなら他の方法でトラッキングする」となれば、それは「3rd Party Cookie Block」という **課題** を解決しただけで、「トラッキング」という **問題** は解決してないことになる。

Fingerprint をする、 CNAME Cloaking をするなど、多くのワークアラウンドが提案されたが、「トラッキング」という **問題** を解決したいベンダからしてみれば、それらを見つけ次第潰していくことになるのは、これまで特に Safari では行っていることを見れば自明だろう。まさしくいたちごっこだ。

仮に PSL への追加が、トラッキングを続けるための課題解決方法として使われるのであれば、同様にこれもどこかで塞がれるだろう。そうしたパッチングは Web をより複雑にし、少しづつ歪が増え、将来的に負債になることは想像に難くない。

かりそめのワークアラウンドのために、 PSL に負荷が集中するのは、 Web を利用する誰にとっても良い結果にはならないと筆者は考えている。
