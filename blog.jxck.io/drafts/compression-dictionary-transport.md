...
ところが、構造化されたデータはヘッダ部分に共通点が増える。つまり「最初ほど似たパターンが多い」という事実もある。
...
つまり、頻出するパターンがあらかじめ分かっているなら、それをまとめておくとより効率よく圧縮できる。 LZ77 では、このパターン集を辞書(Dictionary)と呼ぶ。汎用アルゴリズムを超えた圧縮を実現するには、いかに精度の良い辞書を用意するかが、性能を大きく左右することになる。
...
Brotli が RFC になったのは 2016 年だ。すでに 10 年経ち、その間にも Web には様々な変化がある。その変化を辞書に反映するには、 RFC を更新する必要が出てしまう。
...
Zstd は、辞書を用いる仕組みを保っているが、 Brotli のように仕様の中に辞書を持っているわけではない。代わりに、辞書を作る _"Training Mode"_ を提供している。
