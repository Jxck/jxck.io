...
しかし、動的に HTML を生成し、レスポンスしたら捨ててしまうタイプの実装では相性が悪いため、CDN などに HTML をキャッシュしているような構成の方が相性がいい。辞書にするためメモリに持つ場合は、その管理の戦略も必要だろう。

また、オンザフライで圧縮するため、圧縮率よりは圧縮速度を重視した実装をすることになる。速度を重視する場合は、Brotli よりは Zstd の方が歩がありそうだ。

しかし、このような配信に対応したサーバはざっと探したところ見つかっていない。
...
この `js.dict` をサーバにデプロイし、HTML の `<link>` か HTTP `Link:` ヘッダで指定する。
...
つまり投機的な取得であるため、例えばメインページのための辞書をログインページで落としておくといった設計になるだろう。代わりに、それなりに大きな辞書を配布しやすい。
...
この仕様の特徴として `js.dict` のための `Content-Type` は定義されていない。かなり初期の段階で「あったほうが良いのでは?」という話もしたが、ないまま仕様になってしまった。
...
この仕様の設計は、かつて存在した WebBundle というコンセプトを補助線に考えることができる。

- Webbundle によるサブリソース取得の最適化 | blog.jxck.io
  - https://blog.jxck.io/entries/2020-07-26/bundle-subresources.html

複数の JS を bundle.v1.js にして送る場合、その一部だけが更新された bundle.v2.js も、ファイル全体を送る必要があり、無駄が多かった。

そこで、bundle.v1.js を今のように 1 ファイルに bundle するのではなく、複数ファイルのまま zip で固めて送り、ブラウザ上では個別に取得したかのように振る舞う。ネイティブの `import` で読める。という方式が WebBundle だった。

複数ファイルを WebBundle で送るなら、当然「一部を更新したら、その更新したファイルだけを再取得させたい」という差分更新の仕組みが欲しくなる。そこで、「どのファイルを既に持っているか」をどうやって伝えるかという課題があった。

単純に持っている URL リストなどを送ると、非常にエントロピーの高い情報になり、Fingerprint/Tracking などへの応用ができてしまう。この Privacy Risk の壁を超えられず、WebBundle のコンセプトは失敗した。

ところが、bundle.v1.js を bundle.v2.js の辞書にすることができれば、理想的には v1 と v2 の差分だけを送っている状態にかなり近づけられる。従って、WebBundle のモチベーションの 1 つだった「一度送った情報は、極力再送信しない」という効率的な転送に、かなり近い状態を実現できるのだ。それが CDT のモチベーションの 1 つだった。
...
- `Available-Dictionary` は常に 1 つしか送らない
  ...
  - どれが送られるかはクライアントが選ぶので、トラッキングベクターになりにくい
- CORS が必須
  - 辞書と圧縮対象が全て same-origin である
  - 圧縮対象が現在のページに対して別 Origin なら CORS 必須
...
Static Dictionary の場合、記事ページ(`/entries/*`)から生成した entries.dict を生成し、全ての記事を事前に圧縮しておくことができるだろう。

h2o はどちらの方式にも対応してないため、パッチを充てるか mruby で実装する必要がある。まずは PoC として mruby で実装したいため、比較的対応が楽な後者 Static Dictionary 方式を採用することにした。
...
今回は単純に `/entries/**/*.html` を全てをサンプルとし、辞書を生成した。EJS をテンプレートエンジンとして使っているため、`/template/*.ejs` を辞書にもできるが、記事本文によく出てくるパターンも網羅できるため、素直に全記事をサンプルとした。

作った辞書は `/dictionary/entries.dict` として配布している。記事が変われば辞書も変わるが、辞書はハッシュをチェックしてから使うため、パスを変える必要は無さそうだ。

辞書には固定のヘッダをつける必要がある。

Delta Compression Brotli は以下の 36 byte
...
Delta Compression Zstd は以下の 36 byte
...
辞書は大きくもできるが、大きければ効率が良いわけでもない。色々テストをした結果、256KB を採用した。

辞書もほとんどただのテキストなので、Brotli でキツめに圧縮したところ、60KB ほどになった。
...
提供している辞書に加えて、Brotli 自体が静的な辞書をもっていることも効いているのかもしれない。
...
また、DCT をネゴシエーションできなかった場合は、従来通り `.html.br` を返す。

`.html.br` と `.html.dcb` を比べると以下だ。

つまり辞書のお陰で、ただ brtoli 圧縮するより、半分くらい小さくなっていると言える。


## 辞書の配布

作った辞書は、`blog.jxck.io` 以下全てのパスで、`Link` ヘッダでアドバタイズしている。
...
辞書の有効期限は、単純に辞書の `Cache-Control` で行う。以前は `ttl=3600` のように、キャッシュと独立した辞書の期限を指定できる提案もあった。これは特に、先の Delta Compression のパターンで HTML レスポンスの期限と、辞書としての期限を変えたいという要望だった。しかし、`max-age` と `stale-while-revalidate` の併用などによってカバーできる可能性があり、必要になっら再度議論する方向で一旦削除されている。
...
本当は Cache Busting を考えて、`${sha256}.dict` のようにしたかったが、h2o.conf でヘッダを付ける部分が微妙に面倒になるため、今回は辞書を固定パスにした。
...
- 起動時に `entries.dict` を取得し SHA256 のハッシュを事前に計算しておく
- リクエストごとに以下をチェック
  - `Accept-Encoding: dcb` がある
  - `Available-Dictionary` が `entries.dict` のハッシュと一致
  - `Dictionary_Id` が `entries` と一致
...
一応 Chrome は `chrome://net-internals` で持っている辞書の情報は見ることができ、そこからキャッシュを消すことはできる(名前の通り過去の遺物感ある)。

chrome://net-internals/#sharedDictionary

![chrome のデバッグ画面](./net-internals.png#621x343)

正しい辞書が使えてない場合などは結構ハマる。

次回解説するが、`zstd --train` がそのまま辞書に使えると思ってかなりの時間を溶かした。

もうすこし使われるようになれば、このあたりも改善してくれると思いたい。


## DEMO

本サイトの記事 HTML に限定し、Compression Dictionary Transport の Static Dictionary 対応を行った。

![CDT にり辞書で圧縮された HTML とそのヘッダ](./dictionary-compressed-html.png)
