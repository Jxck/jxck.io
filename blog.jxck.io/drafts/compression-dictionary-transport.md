...
ちょっとだけ違うリソースを連続して取得するような場面では、前回取得したものを辞書にして圧縮を行えば、圧縮率を上げられる。そこで、既に取得したリソースを辞書とし、次に取得するリソースを差分圧縮するのが、Delta Compression だ。

例えば HTML の場合、同じサイトのページは `<main>` 以外の部分に共通点が多くなる。そこで、前のページの HTML を辞書として、次のページの HTML を圧縮すれば、自然と圧縮率は上がるだろう。
...
Separate dictionary の場合、記事ページ(`/entries/*`)から生成した `entries.dict` を生成し、全ての記事を事前に圧縮しておくことができるだろう。

h2o はどちらの方式にも対応してないため、パッチを充てるか mruby で実装する必要がある。まずは PoC として mruby で実装したいため、比較的対応が楽な後者 Separate dictionary 方式を採用することにした。
...
つまり辞書のお陰で、ただ brotli 圧縮するより、半分くらい小さくなっていると言える。
