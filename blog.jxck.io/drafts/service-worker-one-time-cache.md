...
従来 HTTP ヘッダを用いて、ブラウザ自身にキャッシュを管理させる方法を用いてきた。
ここには大きく二つの方式がある。

- ブラウザはリクエストを発行せず、キャッシュを表示する
- ブラウザはリクエストを発行し、キャッシュの鮮度を確かめてから、キャッシュを表示する

前者は `max-age` などの期間で明示されるか、 `last-modified` からブラウザ自身が判断して、前回のキャッシュを使う。
リクエストが無い以上、理論的にこれが最速と言えるが、例えば `max-age` を長く設定してしまうと、その期間内にコンテンツを更新しても反映されなくなる。
ブログで言えば、採用している JS/CSS のライブラリや、更新の少ないロゴ画像などには採用しやすいが、それも更新したい場合はゼロとは言えない。
