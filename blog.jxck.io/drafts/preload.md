...
いくつかのページで試し、結果を貼ろうと思ったが、もとが速すぎるので、もはや誤差のような結果にしかならなかった。

むしろページによっては、むしろ遅くなる場合もあったため、元のコンテンツに特にリソースの読み込みなどに関するオーバーヘッドが少ない場合は、入れることによる内部処理のオーバーヘッドが上回りそうなことがわかった。
...
現状 Chrome Canary しか対応していないこと、このブログは作ってまだ間もないので、コンテンツも少ないこともふまえ。今回は導入を見送った。

今後コンテンツが増え、ボトルネックが発生した際に再度検証し、その時は結果を貼る。
