...
この条件でも、かなり多くのレポートが集まるという点を中心に、その内容やレポーティングサーバについて解説する。
...
そこで、比較的に技術リテラシーが高いユーザが閲覧していると予想される本サイトに対して、去年の 3 月から CSP を適用しレポートの収集を実施することにした。
...
しかし、全体として、定常的にレポートが上がっており、コンテンツが安定ている今でも、レポートがなくなることは基本的には無いことがわかる。
...
この時、 HTML の中にある Inline Style がポリシー違反と判定されるため、レポートが上がる。
...
Chrome でも同じくオリジンが変わるが、ここではレポートはあがらない実装になっているようだ。
...
もちろん、 Report-Only を外してそれらをブロックしていたら、ユーザがどういう体験をしていたかも、ほとんどわからない。
...
それでも、 [github](https://github.com) や [twitter](https://twitter.com) はすでに Report-Only 無しで CSP を運用しているため、ユーザのフィードバックを反映しながら、ポリシーの精度を向上し徐々に適用していけば、 Report-Only なしの運用も決して無理ではないだろう。

一方、レポートが収集できること、それを解析してサービス上で何が起こっているかがわかるだけでも、何もわからなかった従来と比べれば大きな進歩であると感じている。
