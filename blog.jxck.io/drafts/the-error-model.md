...
winerror.h にある S_OK, E_FAULT, SUCCEEDED() などのマクロがあり、 integer をハンドルできる。
windows の最も重要なコードも、 code を返す方式で書かれていた。
kernel には Exception はない。少なくとも意図的ではない。
...
A detailed WG21 paper, Deprecating Exception Specifications, describes how C++ ended up here, and has this to offer in its opening statement:
