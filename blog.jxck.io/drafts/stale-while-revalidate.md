...
したがって「**キャッシュは効かせたいが、なるべく新鮮なリソースを提供したい。**」などといった要望に対処するのが難しかった。
...
しかし、 3600s をすぎるとキャッシュは **stale** とみなされ破棄される。次のリクエストは実際に fetch が走る。
...
なんらかの理由で 360s の間に validate が完了しなければ、キャッシュを **true stale** と皆して破棄し、次のリクエストは fetch が走る。


### stale-if-error

Cache-Control に `stale-if-error` を指定する。
...
1 年間キャッシュが途中で消されない理想状態においては、そのブラウザからサーバへのリクエストは 1 年間無いことになる。
...
しかし、 1 年間はこの stale cache を使用することが許可されているため、次のリクエストはキャッシュヒットするだろう。
...
例えばキャッシュの更新を優先して `1 year stale cache` を指定すると、 `stale-while-revalidate` に ***対応していないブラウザ** の場合、ほぼキャッシュが設定されていないのと同じリクエスト量になってしまう。

この場合、リソースが更新されてから、それが実際適用されるまでのラグは、ユーザのアクセス頻度に依存する。

まず、最初の半年の間にリソースが更新されたと仮定する。これが確実に全体に行き渡るのはいつだろうか。

最初の半年は `max-age` の期間なので、キャッシュは一切更新されない。
半年が過ぎてからのアクセスで初めてキャッシュが更新(validate)されて、その次のリクエストでそのキャッシュがヒットすることでやっと反映されることになる。

つまり、ユーザが頻繁にアクセスしていれば半年過ぎてすぐに新しいものになるが、場合によっては 1 年間更新されないことになる。

ところが、後半の半年、例えば 9 ヶ月(半年 + 3 ヶ月)に更新されたとした場合すこし複雑だ。

後半始まってから 1 ヶ月に stale の状態で fetch が走ったユーザは、古いリソースを同じヘッダで受け取るため、そこでまた半年の fresh キャッシュを持つことになる。
このユーザが次に validate するのは半年後なので、最初のアクセスから 11 ヶ月後にやっとキャッシュが更新されて、その次のアクセスがキャッシュヒットになる。
最悪のケースでは 9 ヶ月目のリソース更新直前に古いキャッシュが更新され、 15 ヶ月目にキャッシュが新しいものに更新され、 21ヶ月目のアクセスでやっと反映されることになる。

従ってもしリソースを更新が計画できるのであれば、その時期に近づくにつれて徐々に max-age を短くしていく必要がある。
もし突発的な更新が発生しても早めに行き渡らせたいのであれば、最初から max-age が短くしておくか、更新で URL を変えるしかない。
...
合わせて、操作ミスなどでブログが落ちていたとしても、その日のうちはキャッシュがが大体表示として十分に機能すると考える。

結果、以下のように設定することとした。


- max-age: 1min SwR 非対応ブラウザではキャッシュしない
- SwR: 1h は、キャッシュを使用(その日見ているセッションの中で)
- SiE: 1day は、エラーの大体表示として stale cache を利用

```
Cache-Control: max-age=1, stale-while-revalidate=300 statle-if-error=864000
```
