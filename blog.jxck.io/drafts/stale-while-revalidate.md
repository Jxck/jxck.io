...
1月1日にユーザがこのヘッダを受け取ったとし、 3月にサーバ側でリソースが更新されたと仮定する。これが確実に全体に行き渡るのはいつだろうか。

まず、6月までは `max-age` の期間なので、キャッシュは一切更新されない。

7月にアクセスしたとすると、最初のキャッシュがヒットするが、裏でキャッシュは更新(validate)される。

すぐまた 7月にアクセスすれば、この時点で3月の更新が反映されるが、もしかしたら次のアクセスは12月かもしれない。

つまり3月の更新は、7月過ぎて2回目のアクセスで新しいものになる、場合によっては12月まで更新されないことになる。


一方、もしサーバ側でのリソースの更新が3月じゃなく、9月だったらどうなるだろうか。
