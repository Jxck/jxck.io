...
template を入れる理由は、最終 HTML に出る共通断片を辞書へ寄せるためです。ただし評価には使いません。
...
`.src/template/*.ejs`, `blog.jxck.io/entries/**/*.html`, `.src/dictionary/dict-generator.rb` に依存して `blog.jxck.io/dictionary/entries.dict` を更新します。
