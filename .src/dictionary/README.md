...
成功時は、実際に書いた辞書ファイルのパスを `stdout` に 1 行だけ出す。`-v` の進捗ログは `stderr` に出すため、Make などから機械的に受け取りやすい。
...
配信される辞書本体は `blog.jxck.io/dictionary/` 直下に置く。
現在の運用では、`make dict` が `blog.jxck.io/dictionary/entries.dict` を直接更新する。
...
./.src/dictionary/compress.sh \
  --dict ./.src/dictionary/work/best.dict \
  --output-dir ./.src/dictionary/work/compressed \
...
for ext in br.br zstd.br rb.br br.zstd zstd.zstd rb.zstd cat.br cat.zstd; do
  total=0
  for f in .src/dictionary/sample/*.html; do
    v=$(wc -c < "${f}.${ext}")
    total=$((total + v))
  done
  echo "$ext: $total"
